...
> **Current baseline (August 2026):** OpenTales now includes first-class scenes, continuous manuscript modes, a command palette, project search/references, an expanded Problems engine, document-based agentic writing, reviewable prose proposals, automated tests/evals/CI, and DOCX/PDF/EPUB/archive publishing. Items below describe further depth unless explicitly labeled unimplemented. Operational behavior is documented in [`architecture.md`](architecture.md), [`ai-system.md`](ai-system.md), and [agentic writing](agentic-writing.md); this file is not a source-of-truth capability list.
...
- Optional **sentence completion** — small local model trained on your own past chapters, so it suggests in _your_ voice rather than ChatGPT's voice.
