...
describe("SearchPanel symbol refactors", () => {
  afterEach(() => {
    ...
    const characterIndex = manuscript.characters.findIndex(
      (character) => character.id === "search-character",
    );
    if (characterIndex >= 0) manuscript.characters.splice(characterIndex, 1);
    const locationIndex = manuscript.locations.findIndex(
  ...
  it("starts rename from a selected character reference target", async () => {
    ...
    await fireEvent.click(
      await screen.findByRole("button", { name: "Rename Mara" }),
    );
    await waitFor(() =>
      expect(storyUi.renameSymbolRequest).toMatchObject({
        targetType: "character",
        targetId: "search-character",
        name: "Mara",
        aliases: ["The Fox"],
      }),
    );
  ...
  it("starts rename from a selected location reference target", async () => {
    ...
    await fireEvent.click(
      await screen.findByRole("button", { name: "Rename North Station" }),
    );
    await waitFor(() =>
      expect(storyUi.renameSymbolRequest).toMatchObject({
        targetType: "location",
