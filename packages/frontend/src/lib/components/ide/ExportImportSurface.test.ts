...
vi.mock("$lib/stores/exportImport.svelte", () => ({
  exportImport: {
    ...
    preview: {
      ...
      sizeBytes: 100,
      chapters: [
        { number: 1, title: "Arrival", body: "Imported body.", scenes: [] },
      ],
      conflicts: [
        {
          kind: "chapter-number",
          sourceKey: "1",
          existingId: "chapter-1",
          message: "Chapter number 1 already exists.",
        },
      ],
      sourceMetadata: {},
...
const { default: ExportImportSurface } = await import(
  "./ExportImportSurface.svelte"
);
...
describe("ExportImportSurface", () => {
  it("submits explicit format, preset and source selection", async () => {
    ...
    await fireEvent.click(
      screen.getByRole("button", { name: "Generate & validate" }),
    );
    expect(mocks.createExport).toHaveBeenCalledWith(
      expect.objectContaining({
        format: "pdf",
        preset: "standard-manuscript",
        target: { kind: "main" },
      }),
    );
  });
  ...
  it("requires explicit conflict confirmation before applying a preview", async () => {
    ...
    const apply = screen.getByRole("button", {
      name: "Apply versioned import",
    }) as HTMLButtonElement;
    expect(apply.disabled).toBe(true);
    await fireEvent.click(
      screen.getByLabelText("Confirm versioned overwrite of matched chapters"),
    );
    expect(apply.disabled).toBe(false);
    await fireEvent.click(apply);
    expect(mocks.applyPreview).toHaveBeenCalledWith(
      expect.objectContaining({
