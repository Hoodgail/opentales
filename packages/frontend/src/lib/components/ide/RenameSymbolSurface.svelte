<script lang="ts">
  ...
  async function navigateToOccurrence(occurrence: RenameSymbolOccurrence) {
    ...
    const directChapter = manuscript.chapters.find(
      (chapter) =>
        chapter.id === occurrence.entityId ||
        chapter.writingId === occurrence.writingId,
    );
    const sceneChapter =
      directChapter ??
      manuscript.chapters.find((chapter) =>
        chapter.scenes.some(
          (scene) =>
            scene.id === occurrence.entityId ||
            scene.writingId === occurrence.writingId,
        ),
      );
    if (sceneChapter) {
      const line =
        directChapter && occurrence.field === "body"
    ...
    if (
      occurrence.entityType === "story-structure" ||
      occurrence.entityType === "obstacle"
    ) {
      await manuscript.openTab({
        id: "tab-structure",
        type: "structure",
    ...
    const character = manuscript.characters.find(
      (item) => item.id === occurrence.entityId,
    );
    if (character) {
...
{#if storyUi.renameSymbolRequest && manuscript.projectId}
  <RenameSymbolDialog
    ...
    onPreview={(input) =>
      storyIde.previewRenameSymbol(manuscript.projectId!, input)}
    onApply={(input) =>
      storyIde.applyRenameSymbol(manuscript.projectId!, input)}
