...
<aside class="flex min-h-0 w-full shrink-0 flex-col border-t border-border bg-sidebar xl:w-[22rem] xl:border-l xl:border-t-0" aria-label="Build task details">
  ...
  {:else}
    ...
    <footer class="flex shrink-0 gap-1 border-t border-border p-2">
      {#if task.status === 'failed' && task.attempts < task.maxAttempts}<button type="button" disabled={busy} onclick={() => onRetry(task)} class="inline-flex flex-1 items-center justify-center gap-1 rounded bg-accent px-2 py-1.5 text-[10px] font-medium text-accent-foreground outline-none disabled:opacity-50 focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 focus-visible:ring-offset-sidebar"><RefreshCw class="size-3" />Retry task</button>{/if}
      <button type="button" disabled={busy || task.status === 'running'} onclick={(event) => onRerun(task, event)} title="Rerun this task and invalidate transitive downstream output" class={cn('inline-flex flex-1 items-center justify-center gap-1 rounded px-2 py-1.5 text-[10px] font-medium outline-none disabled:opacity-40 focus-visible:ring-2', task.status === 'failed' && task.attempts >= task.maxAttempts ? 'bg-amber-400 text-black focus-visible:ring-amber-300' : 'border border-border text-foreground hover:border-accent/50 focus-visible:ring-accent')}><Wrench class="size-3" />{task.status === 'failed' && task.attempts >= task.maxAttempts ? 'Rerun failed boundary' : 'Rerun'}</button>
