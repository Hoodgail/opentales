<script lang="ts">
  ...
  import {
    editorTheme,
    type EditorThemeId,
  } from "$lib/stores/editorTheme.svelte";
  ...
  let commands = $derived.by<CommandItem[]>(() => {
    ...
    for (const ch of manuscript.chapters) {
      out.push({
        ...
        subtitle:
          ch.number === 0
            ? "Prologue"
            : `Chapter ${ch.number} · ${ch.wordCount.toLocaleString()} words`,
        icon: FileText,
    ...
    out.push({
      id: "action:export-manuscript",
      kind: "action",
      title: "Export manuscript…",
      subtitle: "DOCX, PDF, EPUB, Markdown, text, HTML, or project archive",
      icon: Archive,
      keywords: "publish compile download docx pdf epub export",
      run: () => {
        manuscript.setActiveView("publishing");
        void manuscript.openTab({
          id: "tab-publishing",
          type: "publishing",
          refId: "publishing",
    ...
    out.push({
      id: "action:import-manuscript",
      kind: "action",
      title: "Import manuscript…",
      subtitle: "Preview DOCX, Markdown, text, HTML, or OpenTales archive",
      icon: Archive,
      keywords: "upload preview conflict docx markdown import",
      run: () => {
        manuscript.setActiveView("publishing");
        void manuscript.openTab({
          id: "tab-publishing",
    ...
    out.push({
      id: "action:move-scene",
      kind: "action",
      title: "Move scene…",
      subtitle:
        "Open the persisted Corkboard and drag a scene within its chapter",
      icon: GitBranch,
      keywords: "reorder move scene corkboard",
      run: () => {
    ...
    out.push({
      id: "action:compare-snapshot",
      kind: "action",
      title: "Compare with snapshot…",
      subtitle: "Open immutable prose and semantic history",
      icon: History,
      keywords: "revision history diff restore branch snapshot",
      run: () => {
        manuscript.setActiveView("revisions");
        void manuscript.openTab({
          id: "tab-revisions",
          type: "revisions",
          refId: "revisions",
    ...
    const activeTab = manuscript.tabs.find(
      (tab) => tab.id === manuscript.activeTabId,
    );
    const activeChapter =
      activeTab?.type === "chapter"
        ...
        : null;
    if (activeChapter) {
      const selectedSceneIndex = activeChapter.scenes.findIndex(
        (scene) => scene.id === storyUi.selectedOutlineSceneId,
      );
      out.push({
        ...
        subtitle:
          selectedSceneIndex >= 0
            ? `Insert after ${activeChapter.scenes[selectedSceneIndex].title}`
            : `Append to ${activeChapter.title}`,
        icon: GitBranch,
        keywords: "new scene metadata plan",
        run: async () => {
          await manuscript.createScene(activeChapter.id, {
    ...
    const selectedChapter = manuscript.chapters.find(
      (item) => item.id === manuscript.selectedId,
    );
    const selectedCharacter = manuscript.characters.find(
      (item) => item.id === manuscript.selectedId,
    );
    const selectedLocation = manuscript.locations.find(
      (item) => item.id === manuscript.selectedId,
    );
    const renameTarget = selectedCharacter
      ...
      : selectedLocation
        ? {
            ...
            aliases: selectedLocation.aliases,
          }
        : null;
    ...
    out.push({
      ...
      kind: "action",
      title: reference
        ? `Find references to ${reference.title}`
        : "Find references…",
      subtitle: reference
        ? "Traverse the persistent story index"
        : "Select a chapter, character, or location first",
      icon: Search,
      keywords: "find references backlinks usages symbol",
      run: () => {
        if (reference)
          storyUi.requestReferences(
            reference.type,
            reference.id,
            reference.title,
          );
  ...
  let filtered = $derived.by<CommandItem[]>(() => {
    ...
    for (const cmd of commands) {
      const haystack =
        `${cmd.title} ${cmd.subtitle ?? ""} ${cmd.keywords ?? ""}`.toLowerCase();
      let score = 0;
      let matchAll = true;
