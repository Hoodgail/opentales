...
describe("RenameSymbolDialog", () => {
  it("shows an exact diff and requires reviewed confirmation before applying CAS inputs", async () => {
    ...
    render(RenameSymbolDialog, {
      props: {
        ...
        builds: [
          { id: "build-1", branchName: "ai/alternate", status: "paused" },
        ],
    ...
    await fireEvent.input(screen.getByRole("textbox", { name: "New name" }), {
      target: { value: "Maris" },
    });
    ...
    expect(
      await screen.findByText("When Mara crossed the bridge"),
    ).toBeTruthy();
    ...
    await fireEvent.click(
      screen.getByRole("checkbox", { name: /I reviewed this exact preview/ }),
    );
  ...
  it("uses main manuscript scope and blocks conflicted previews", async () => {
    const onPreview = vi.fn().mockResolvedValue({
      ...preview,
      scope: "all",
      buildRunId: null,
      conflicts: [
        "Another character already uses 'Maris' as a canonical name or alias",
      ],
    });
    const onApply = vi.fn();
    render(RenameSymbolDialog, {
    ...
    expect((await screen.findByRole("alert")).textContent).toMatch(
      /Resolve conflicts before\s+applying/,
    );
    await fireEvent.click(
      screen.getByRole("checkbox", { name: /I reviewed this exact preview/ }),
    );
  ...
  it("reuses the exact apply idempotency key after an ambiguous response failure", async () => {
    ...
    const reviewed = await screen.findByRole("checkbox", {
      name: /I reviewed this exact preview/,
    });
