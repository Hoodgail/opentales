...
>
  <nav
    class="min-h-0 flex-1 overflow-y-auto overflow-x-hidden [scrollbar-width:none] flex flex-col items-center gap-0.5"
  ...
  >
    {#each items as item (item.id)}
      ...
      <button
        ...
        title={active && !ui.sidePanelCollapsed
          ? `Collapse ${item.label}`
          : item.label}
        class={cn(
  ...
  <div
    class="mt-1 flex shrink-0 flex-col items-center gap-0.5 border-t border-border/60 pt-1"
  >
    <button
      type="button"
      onclick={() => ui.toggleSidePanel()}
      title={ui.sidePanelCollapsed
        ? "Expand side panel"
        : "Collapse side panel"}
      aria-label={ui.sidePanelCollapsed
        ? "Expand side panel"
        : "Collapse side panel"}
