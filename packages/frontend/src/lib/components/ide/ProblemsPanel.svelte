<script lang="ts">
  ...
  const enabledRuleIds = $state<Set<string>>(
    new Set(
      allRules.filter((rule) => rule.defaultEnabled).map((rule) => rule.id),
    ),
  );
  ...
  const diagnostics = $derived(localDiagnostics);
  ...
  const categories = $derived(
    Array.from(
      new Set(
        diagnostics.map((diagnostic) => diagnostic.category).filter(Boolean),
      ),
...
<div class="flex h-full flex-col">
  ...
  <div
    class="flex items-center gap-2 border-b border-border px-3 py-2 text-[11px]"
  >
    <button
      type="button"
      onclick={() =>
        (severityFilter = severityFilter === "error" ? "all" : "error")}
      aria-label={`${counts.error} errors`}
      aria-pressed={severityFilter === "error"}
      class="flex items-center gap-1 rounded px-1 py-0.5 text-destructive outline-none hover:bg-muted focus-visible:ring-2 focus-visible:ring-accent"
    >
    ...
    <button
      type="button"
      onclick={() =>
        (severityFilter = severityFilter === "warning" ? "all" : "warning")}
      aria-label={`${counts.warning} warnings`}
      aria-pressed={severityFilter === "warning"}
      class="flex items-center gap-1 rounded px-1 py-0.5 text-amber-400 outline-none hover:bg-muted focus-visible:ring-2 focus-visible:ring-accent"
    >
    ...
    <button
      type="button"
      onclick={() =>
        (severityFilter = severityFilter === "info" ? "all" : "info")}
      aria-label={`${counts.info} informational issues`}
      aria-pressed={severityFilter === "info"}
      class="flex items-center gap-1 rounded px-1 py-0.5 text-muted-foreground outline-none hover:bg-muted focus-visible:ring-2 focus-visible:ring-accent"
    >
      <Info class="size-3" />
      {counts.info}
    </button>
    <button
      ...
      class={cn(
        "ml-auto inline-flex items-center gap-1 rounded border border-border px-2 py-0.5 text-[10px] outline-none hover:bg-muted focus-visible:ring-2 focus-visible:ring-accent",
        categoryFilter !== "all" || passFilter !== "all"
          ? "text-accent"
    ...
    <button
      ...
      class="rounded border border-border px-2 py-0.5 text-[10px] text-muted-foreground outline-none hover:bg-muted focus-visible:ring-2 focus-visible:ring-accent"
      >Rules</button
    >
    <button
      type="button"
      ...
      class="rounded p-1 text-muted-foreground outline-none hover:bg-muted focus-visible:ring-2 focus-visible:ring-accent"
  ...
  {#if filterOpen}
    <div class="grid grid-cols-1 gap-2 border-b border-border bg-card/35 p-2">
      <label>
        <span
          class="mb-1 block text-[9px] font-semibold uppercase tracking-wider text-muted-foreground"
          >Category</span
        >
        <select
          bind:value={categoryFilter}
          class="h-8 w-full rounded border border-border bg-input/70 px-2 text-[11px] text-foreground outline-none focus-visible:border-accent focus-visible:ring-2 focus-visible:ring-accent/30"
        >
      ...
      <label>
        ...
        >
        <select
          bind:value={passFilter}
          class="h-8 w-full rounded border border-border bg-input/70 px-2 text-[11px] text-foreground outline-none focus-visible:border-accent focus-visible:ring-2 focus-visible:ring-accent/30"
        >
          <option value="all">All passes</option>
          {#each passOptions as pass (pass.value)}<option value={pass.value}
              >{pass.label}</option
            >{/each}
        </select>
      </label>
  ...
  {#if configOpen}
    <div
      class="space-y-1 border-b border-border bg-card/40 px-3 py-2 text-[11px]"
    >
      {#each allRules as rule (rule.id)}
        <label class="flex cursor-pointer items-start gap-2">
          ...
          <span class="min-w-0 flex-1"
            ><span class="text-foreground">{rule.label}</span><span
              class="ml-1 text-muted-foreground">{rule.description}</span
            ></span
          >
        </label>
      {/each}
    ...
    {#if grouped.length === 0}
      <div class="px-4 py-12 text-center text-xs text-muted-foreground">
        {diagnostics.length === 0
          ? "No problems detected in the current manuscript."
          : "No diagnostics match this revision pass."}
      </div>
    {/if}
    {#each grouped as group (group.category)}
      <section
        class="border-b border-border"
        aria-labelledby={`problem-group-${group.category}`}
      >
        <div
          id={`problem-group-${group.category}`}
          class="flex items-center justify-between bg-card/40 px-3 py-1 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground"
        >
          <span>{group.category.replace("-", " ")}</span><span class="font-mono"
          ...
          <div class="border-b border-border/40 last:border-b-0">
            <div class="flex items-start gap-1 px-2 py-2">
              <button
                ...
                aria-label="Show diagnostic evidence"
                class="mt-0.5 rounded p-0.5 text-muted-foreground outline-none hover:text-foreground focus-visible:ring-2 focus-visible:ring-accent"
              >
                {#if expandedId === id}<ChevronDown
                    class="size-3"
              ...
              <button
                type="button"
                onclick={() => openPrimaryEvidence(diagnostic)}
                class="min-w-0 flex-1 text-left outline-none focus-visible:ring-2 focus-visible:ring-accent/40"
              >
                <span class="text-xs text-foreground">{diagnostic.message}</span
                >
                {#if diagnostic.hint}<span
                    class="mt-0.5 block text-[11px] leading-relaxed text-muted-foreground"
                    >{diagnostic.hint}</span
                  >{/if}
                <span
                  class="mt-1 flex flex-wrap gap-x-2 font-mono text-[9px] uppercase text-muted-foreground/70"
                >
                  <span>{diagnostic.ruleId}</span>
                  <span>{diagnostic.source ?? "local"}</span>
                  {#if diagnostic.pass}<span>pass:{diagnostic.pass}</span>{/if}
                  {#if diagnostic.lineStart}<span
                      >line:{diagnostic.lineStart}</span
                    >{/if}
                </span>
              </button>
            </div>
              ...
              <div class="border-t border-border/30 bg-background/45 px-8 py-2">
                ...
                {:else}
                  <p class="text-[10px] text-muted-foreground">
                    This rule did not emit source evidence.
                  </p>
                {/if}
              </div>
            {/if}
