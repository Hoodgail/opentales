<script lang="ts">
  import { Bot, ChevronDown, ChevronRight, Loader2, Paperclip, User, Wrench } from 'lucide-svelte';
  ...
  type TimelineItem =
    | { type: 'message'; id: string; createdAt: string; message: AiAgentMessage }
    | { type: 'tool'; id: string; createdAt: string; toolCall: AiAgentToolCall };
  ...
  function buildTimeline(value: AiAgentSession | null): TimelineItem[] {
    ...
    return [
      ...value.messages
        ...
        .map((message) => ({ type: 'message' as const, id: `message-${message.id}`, createdAt: message.createdAt, message })),
      ...value.toolCalls.map((toolCall) => ({ type: 'tool' as const, id: `tool-${toolCall.id}`, createdAt: toolCall.createdAt, toolCall }))
  ...
  <div class="space-y-3 p-3">
    {#each timelineItems as item (item.id)}
      ...
      {:else}
        ...
        <div class="ml-7 rounded-md border border-border/60 bg-muted/10 text-[11px] text-muted-foreground">
          <button
            type="button"
          ...
          >
            ...
            <span class="min-w-0 flex-1 truncate">
              ...
              {#if toolOutputLabel(tc.output)}<span> · {toolOutputLabel(tc.output)}</span>{/if}
              {#if tc.error}<span class="text-destructive"> · {tc.error}</span>{/if}
            </span>
          ...
          {#if expandedTools[tc.id]}
            <div class="grid gap-1 px-2 pb-2">
              <pre class="max-h-40 overflow-auto rounded bg-background/70 p-2 text-[10px] leading-relaxed">{JSON.stringify(tc.input, null, 2)}</pre>
              {#if tc.output !== null && tc.output !== undefined}
                <pre class="max-h-40 overflow-auto rounded bg-background/70 p-2 text-[10px] leading-relaxed">{JSON.stringify(tc.output, null, 2)}</pre>
              {/if}
            </div>
          {/if}
        </div>
