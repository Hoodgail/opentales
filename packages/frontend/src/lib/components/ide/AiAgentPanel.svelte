<script lang="ts">
  ...
  import type { AiAgentAttachmentInput, AiAgentMessage, AiAgentToolCall, AssetKind } from '@opentales/sdk';
  ...
  const activeModel = $derived(selectedModel || ai.settings?.model || modelOptions[0]);
  ...
  function send() {
    ...
    void ai.queuePrompt(projectId, prompt.trim(), false, { model: activeModel, attachments: queuedAttachments });
    prompt = '';
    attachments = [];
  }
  ...
  function interrupt() {
    if (!projectId || !prompt.trim()) return;
    const queuedAttachments = [...attachments];
    void ai.queuePrompt(projectId, prompt.trim(), true, { model: activeModel, attachments: queuedAttachments });
    prompt = '';
    attachments = [];
  }
  ...
  function modelChoices(settingsModel: string | undefined): string[] {
    const defaults = ['openai/gpt-5.4', 'openai/gpt-5-mini', 'openai/gpt-4o', 'anthropic/claude-sonnet-4.5', 'google/gemini-3-pro-preview'];
    return [...new Set([settingsModel, ...defaults].filter((value): value is string => Boolean(value)))];
  }
...
<div class="flex h-full flex-col">
  ...
  {:else}
          ...
          <ul class="space-y-0.5">
            {#each session.queue.filter((q) => q.status === 'queued') as q (q.id)}
              <li class="truncate pl-2.5 text-[11px] text-muted-foreground">
                {#if q.model}<span class="text-foreground/70">{q.model}</span> · {/if}{q.prompt}
              </li>
            {/each}
          </ul>
        </div>
    ...
    <!-- Input -->
    <div class="border-t border-border bg-sidebar/70 p-2">
      <div class="rounded-xl border border-border bg-background/95 shadow-sm focus-within:border-accent/70">
        {#if attachments.length > 0}
          <div class="flex flex-wrap gap-1.5 border-b border-border/70 px-2 py-2">
        ...
        ></textarea>
        <div class="flex items-center justify-between gap-2 border-t border-border/60 px-2 py-1.5">
      ...
      <div class="mt-1.5 flex items-center justify-between gap-2 text-[10px] text-muted-foreground">
        <span class="inline-flex items-center gap-1">
          <span class="size-1.5 rounded-full {ai.streaming ? 'animate-pulse bg-emerald-400' : 'bg-muted-foreground'}"></span>
          {ai.streaming ? 'Connected' : 'Disconnected'} · {session?.status ?? 'idle'}
        </span>
        {#if session?.contextUsage}
          <span class="min-w-24 text-right" title={`${session.contextUsage.totalTokens} / ${session.contextUsage.maxTokens} tokens`}>
            Context {session.contextUsage.percentage}% · {formatUsage(session.contextUsage.totalTokens)} tokens
          </span>
        {:else}
