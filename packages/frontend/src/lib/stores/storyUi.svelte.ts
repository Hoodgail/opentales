...
function createStoryUiStore() {
  ...
  let continuousStatus = $state<
    "all" | "draft" | "in-progress" | "review" | "final"
  >("all");
  let selectedOutlineSceneId = $state<string | null>(null);
  let renameSymbolRequest = $state<
    (RenameSymbolTarget & { nonce: number }) | null
  >(null);

  return {
    get bibleSelection() {
      return bibleSelection;
    },
    selectBible(next: BibleSelection | null) {
      bibleSelection = next;
    },
    get outlineProjection() {
      return outlineProjection;
    },
    setOutlineProjection(next: OutlineProjection) {
      outlineProjection = next;
    },
    ...
    requestReferences(refType: string, refId: string, title: string) {
      referenceRequest = {
        ...
        refId,
        title,
        nonce: (referenceRequest?.nonce ?? 0) + 1,
      };
    },
    ...
    setContinuousMode(next: "write" | "read" | "source") {
      continuousMode = next;
    },
    get continuousStatus() {
      return continuousStatus;
    },
    setContinuousStatus(
      next: "all" | "draft" | "in-progress" | "review" | "final",
    ) {
      continuousStatus = next;
    },
    get selectedOutlineSceneId() {
      return selectedOutlineSceneId;
    },
    selectOutlineScene(id: string | null) {
      selectedOutlineSceneId = id;
    },
    get renameSymbolRequest() {
      return renameSymbolRequest;
    },
    requestRenameSymbol(target: RenameSymbolTarget) {
      renameSymbolRequest = {
        ...target,
        nonce: (renameSymbolRequest?.nonce ?? 0) + 1,
      };
    },
    clearRenameSymbolRequest() {
