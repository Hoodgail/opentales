...
describe("OpenTales MCP capability surface", () => {
  it("exposes every applicable workspace tool to read-write keys with accurate annotations", async () => {
    ...
    expect(
      tools.some((tool) => /Build|ArtifactBatch|Checkpoint/.test(tool.name)),
    ).toBe(false);
    expect(
      tools.find((tool) => tool.name === "readChapter")?.annotations
        ?.readOnlyHint,
    ).toBe(true);
    expect(
      tools.find((tool) => tool.name === "createChapter")?.annotations
        ?.readOnlyHint,
    ).toBe(false);
    expect(
      tools.find((tool) => tool.name === "deleteChapter")?.annotations
        ?.destructiveHint,
    ).toBe(true);
    expect(
      tools.find((tool) => tool.name === "applyStoryPatch")?.annotations
        ?.idempotentHint,
    ).toBe(true);
    expect(
      tools.find((tool) => tool.name === "compileChapterFromScenes")
        ?.annotations?.idempotentHint,
    ).toBe(true);
    expect(
      tools.find((tool) => tool.name === "updateSubmission")?.annotations
        ?.readOnlyHint,
    ).toBe(false);
    expect(tools.some((tool) => tool.name === "task")).toBe(false);
    expect(tools.some((tool) => tool.name === "askUser")).toBe(false);
    expect(tools.some((tool) => tool.name === "applyBuildUnitPatch")).toBe(
      false,
    );
  });
  ...
  it("removes all mutation tools for read-only keys", async () => {
    ...
    expect(names).toContain("readProjectDoc");
  ...
  it("publishes workspace, agent, and skill prompts plus project resource templates", async () => {
    ...
    expect(
      (prompts.prompts as Array<{ name: string }>).map((prompt) => prompt.name),
    ).toEqual(["opentales_workspace", "opentales_agent", "opentales_skill"]);
    ...
    const uriTemplates = (
      templates.resourceTemplates as Array<{ uriTemplate: string }>
  ...
  it("puts the project scope and progressive skill loading into server instructions", async () => {
    ...
    expect(result.instructions).toContain(
      'permanently scoped to project "The Lantern Book"',
    );
...
async function request(
  ...
  const handler = createMcpHandler(({ authInfo: requestAuth }) =>
    createOpenTalesMcpServer({} as PrismaClient, requestAuth),
  );
