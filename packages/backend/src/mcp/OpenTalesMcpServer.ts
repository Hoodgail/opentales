...
function serverInstructions(auth: McpAuthContext): string {
  ...
  return [
    ...
    'Use opentales_workspace to load author instructions and the skill catalog. Use opentales_agent or opentales_skill to load a specialized prompt. Resolve opaque IDs with list tools. Before changing prose, read the target and copy its headVersionId (plus revision for scenes/build units). To write canonical chapters directly, use createChapter with content, or readChapter then updateChapter with content and expectedHeadVersionId. For applyStoryPatch use patch.mode=replace to initialize an empty body. Use exact edits for bounded changes. Notes, summaries, and planning artifacts are not chapter prose; verify saved chapters with listChapters and readChapter before reporting completion. Do not claim a mutation succeeded until its receipt confirms the new token.',
