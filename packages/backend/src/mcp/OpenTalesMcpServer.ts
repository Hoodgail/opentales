...
import type {
  ApprovalHandler,
  QuestionHandler,
} from "../useCases/ai/tools/mutations.js";
...
const MAX_TOOL_RESPONSE_CHARS = positiveInteger(
  process.env.MCP_MAX_TOOL_RESPONSE_CHARS,
  100_000,
);
...
function registerWorkspaceTools(
  ...
  const unavailableQuestion = {
    async handleQuestion(): Promise<never> {
      throw new HttpError(
        400,
        "askUser is not available over MCP; ask the user through the host agent",
      );
    },
  } as QuestionHandler;
  const unavailableTask = {
    async handleTask(): Promise<never> {
      throw new HttpError(
        400,
        "task is an internal OpenTales agent-session tool; use the MCP agent prompts or the host agent delegation system",
      );
    },
  } as TaskHandler;
  ...
  for (const [name, rawTool] of Object.entries(tools)) {
    ...
    if (!runtimeTool.inputSchema || typeof runtimeTool.execute !== "function")
      continue;
...
function registerWorkspaceResources(
  ...
  server.registerResource(
    ...
    new ResourceTemplate("opentales://skills/{name}", {
      list: async () => ({
        resources: (await loadAiSkillCatalog(prisma, auth.projectId)).map(
          (skill) => ({
      ...
      complete: {
        name: async (value) =>
          (await loadAiSkillCatalog(prisma, auth.projectId))
    ...
    }),
    {
      ...
      description:
        "Full built-in or project Agent Skill instructions, with bundled references when present.",
      mimeType: "text/markdown",
      cacheHint: { ttlMs: 30_000, cacheScope: "private" },
    },
    async (uri, variables) => {
      ...
      if (!skill)
        throw new HttpError(404, `Agent Skill '${name}' not found or disabled`);
  ...
  server.registerResource(
    ...
    new ResourceTemplate("opentales://agents/{name}", {
      list: async () => ({
        resources: (await loadAiAgents(prisma, auth.projectId)).map(
          (agent) => ({
      ...
      complete: {
        name: async (value) =>
          (await loadAiAgents(prisma, auth.projectId))
    ...
    async (uri, variables) => {
      const name = variable(variables.name);
      const agent = (await loadAiAgents(prisma, auth.projectId)).find(
  ...
  server.registerResource(
    ...
    new ResourceTemplate("opentales://instructions/{id}", {
      list: async () => ({
        resources: (await listInstructionDocs(prisma, auth.projectId)).map(
          (doc) => ({
    ...
    }),
    {
      ...
      description:
        "An author-owned INSTRUCTIONS document for the active project.",
      mimeType: "text/markdown",
...
function registerWorkspacePrompts(
  server: McpServer,
  prisma: PrismaClient,
  auth: McpAuthContext,
): void {
  server.registerPrompt(
    "opentales_workspace",
    {
      ...
      description:
        "Load project identity, author instructions, and the skill catalog before handling a writing task.",
      argsSchema: z.object({
        task: z
          .string()
          .trim()
          .max(20_000)
          .optional()
          .describe("The writing or workspace task to perform."),
      }),
    },
    async ({ task }) => ({
  ...
  server.registerPrompt(
    "opentales_agent",
    {
      ...
      description:
        "Load one built-in or project-defined agent persona and apply it to a focused task.",
      argsSchema: z.object({
        name: z
          .string()
          .trim()
          .min(1)
          .describe("Agent name from opentales://agents/{name}."),
        task: z
          .string()
          .trim()
          .max(20_000)
          .optional()
          .describe("Focused task for the selected agent."),
      }),
    },
    async ({ name, task }) => {
      const agent = (await loadAiAgents(prisma, auth.projectId)).find(
  ...
  server.registerPrompt(
    "opentales_skill",
    {
      ...
      description:
        "Load a full built-in or project skill, including bundled references, for a specific task.",
      argsSchema: z.object({
        name: z
          .string()
          .trim()
          .min(1)
          .describe("Skill name from opentales://skills/{name}."),
        task: z
          .string()
          .trim()
          .max(20_000)
          .optional()
          .describe("Task to perform with the selected skill."),
      }),
    },
    async ({ name, task }) => {
      const skill = await readAiSkillFromCatalog(prisma, auth.projectId, name);
...
function serverInstructions(auth: McpAuthContext): string {
  ...
  return [
    ...
    "Use opentales_workspace to load author instructions and the skill catalog. Use opentales_agent or opentales_skill to load a specialized prompt. Resolve opaque IDs with list tools. Before changing prose, read the target and copy its headVersionId (plus revision for scenes). To write canonical chapters directly, use createChapter with content, or readChapter then updateChapter with content and expectedHeadVersionId. For applyStoryPatch use patch.mode=replace to initialize an empty body. Use exact edits for bounded changes. Notes, summaries, and planning docs are not chapter prose; verify saved chapters with listChapters and readChapter before reporting completion. Do not claim a mutation succeeded until its receipt confirms the new token.",
    "Edit an OPEN proposal with updateSubmission or applyStoryPatch instead of opening a duplicate. Keep plans and progress in project docs, and use ordinary writing tools to execute and revise the plan.",
  ].join("\n\n");
}
...
function toolResult(output: unknown): CallToolResult {
  ...
  const previewCharacters = Math.max(
    1_000,
    Math.floor(MAX_TOOL_RESPONSE_CHARS / 2),
  );
  const truncated = {
    truncated: true,
    ...
    guidance:
      "Use pagination, filters, grep, or bounded read ranges to request a smaller result.",
  };
  return {
...
function toolError(toolName: string, error: unknown): CallToolResult {
  if (error instanceof HttpError) {
    const details =
      error.details === undefined ? "" : ` Details: ${jsonText(error.details)}`;
    return {
      isError: true,
      content: [
        {
          type: "text",
          text: `${toolName} failed: ${error.message}.${details}`,
        },
      ],
    };
  }
...
function mcpToolDescription(description: string, readOnly: boolean): string {
  ...
  return (
    description
      .replace(
        /\s*The user will approve\/reject the proposal in the UI\.?/g,
        "",
      )
      .replace(
        /\s*This is approval-gated outside an authorized build scope\.?/g,
        "",
      )
...
function agentMarkdown(
  agent: Awaited<ReturnType<typeof loadAiAgents>>[number],
): string {
  return [
    "---",
    `name: ${agent.name}`,
