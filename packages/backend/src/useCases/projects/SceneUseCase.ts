import {
  Prisma,
  type PrismaClient,
  type SceneStatus as PrismaSceneStatus,
} from "@prisma/client";
import type {
  CreateSceneInput,
  DeleteSceneInput,
...
import { assertJsonValue } from "../storyState/schemas.js";
...
export class SceneUseCase {
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      const existing = await tx.scene.findFirst({
        where: {
          id: sceneId,
          chapterId,
          chapter: { projectId, deletedAt: null },
        },
      ...
      if (existing.revision !== normalized.expectedRevision) {
        throw new HttpError(409, "Scene revision is stale", {
      ...
      if (normalized.status !== undefined)
        data.status = STATUS_MAP[normalized.status];
      if (normalized.povCharacterId !== undefined)
        data.povCharacter =
          normalized.povCharacterId === null
            ? { disconnect: true }
            : { connect: { id: normalized.povCharacterId } };
      if (normalized.locationId !== undefined)
        data.location =
          normalized.locationId === null
            ? { disconnect: true }
            : { connect: { id: normalized.locationId } };
      ...
      if (normalized.order !== undefined) {
        ...
        orderedIds.splice(
          clamp(normalized.order, 0, orderedIds.length),
          0,
          sceneId,
        );
        await this.reorderSequential(tx, chapterId, orderedIds);
  ...
  async reorder(
    ...
    if (
      ...
      throw new HttpError(
        400,
        "sceneIds must be a non-empty unique ordered list",
      );
    }
    let completed = false;
      ...
      try {
        await this.prisma.$transaction(
          async (tx) => {
            await this.lockChapter(tx, projectId, chapterId);
            if (input.buildRunId) {
              const build = await tx.buildRun.findFirst({
                where: { id: input.buildRunId, projectId },
                select: { status: true },
              });
              if (!build)
                throw new HttpError(
                  400,
                  "Reorder buildRunId does not belong to this project",
                );
              if (["COMPLETED", "CANCELLED"].includes(build.status))
                throw new HttpError(
                  409,
                  "Historical or terminal build mappings cannot be reordered",
                );
            }
            ...
            if (
              currentScenes.length !== input.sceneIds.length ||
              currentScenes.some((scene) => !input.sceneIds.includes(scene.id))
            )
              throw new HttpError(
                409,
                "Bulk reorder must include every scene in the chapter exactly once",
              );
            for (const scene of currentScenes)
              if (
                !Number.isInteger(input.expectedRevisions?.[scene.id]) ||
                input.expectedRevisions[scene.id] !== scene.revision
              ) {
                throw new HttpError(
                  409,
                  `Scene '${scene.id}' revision is stale`,
                  {
                    expected: input.expectedRevisions?.[scene.id],
                    actual: scene.revision,
                  },
                );
              }
            ...
            const rows = input.sceneIds.map(
              (id, order) => Prisma.sql`(${id}::text, ${order}::integer)`,
            );
            await tx.$executeRaw(
              Prisma.sql`UPDATE "Scene" AS scene SET "order"=ordered."order", revision=scene.revision+1, "updatedAt"=CURRENT_TIMESTAMP FROM (VALUES ${Prisma.join(rows)}) AS ordered(id,"order") WHERE scene.id=ordered.id AND scene."chapterId"=${chapterId}`,
            );
            const mapped = input.buildRunId
              ? await tx.buildManuscriptUnit.findMany({
                  where: {
                    projectId,
                    buildRunId: input.buildRunId,
                    sourceSceneId: { in: input.sceneIds },
                    invalidatedAt: null,
                  },
                  select: { id: true, sourceSceneId: true },
                })
              : [];
            if (mapped.length) {
              await tx.buildManuscriptUnit.updateMany({
                where: { id: { in: mapped.map((unit) => unit.id) } },
                data: { order: { increment: 1_000_000 } },
              });
              const unitRows = mapped.map(
                (unit) =>
                  Prisma.sql`(${unit.id}::text, ${input.sceneIds.indexOf(unit.sourceSceneId!)}::integer)`,
              );
              await tx.$executeRaw(
                Prisma.sql`UPDATE "BuildManuscriptUnit" AS unit SET "order"=ordered."order", revision=unit.revision+1, "updatedAt"=CURRENT_TIMESTAMP FROM (VALUES ${Prisma.join(unitRows)}) AS ordered(id,"order") WHERE unit.id=ordered.id AND unit."buildRunId"=${input.buildRunId}`,
              );
            }
          },
        ...
        if (attempt === 7)
          throw new HttpError(
            409,
            "Concurrent scene reorder could not be serialized; reload revisions and retry",
          );
        await new Promise((resolve) =>
          setTimeout(
            resolve,
            15 * (attempt + 1) + Math.floor(Math.random() * 20),
          ),
        );
      }
    }
  ...
  private async assertReferences(
    ...
    if (characterIds.length) {
      ...
      if (count !== characterIds.length)
        throw new HttpError(
          400,
          "One or more character references do not belong to this project",
        );
    }
    if (input.locationId) {
    ...
    if (threadIds.length) {
      const count = await tx.plotThread.count({
        where: {
          projectId,
          id: { in: threadIds },
          isCurrent: true,
          invalidatedAt: null,
          buildRun: { reviews: { some: { status: "MERGED" } } },
        },
      });
      if (count !== threadIds.length)
        throw new HttpError(
          400,
          "Canonical scenes may reference only current plot threads from an explicitly merged build",
        );
    }
    const setupIds = unique(input.setupPayoffIds ?? []);
    if (setupIds.length) {
      const count = await tx.setupPayoffLink.count({
        where: {
          projectId,
          id: { in: setupIds },
          isCurrent: true,
      ...
      if (count !== setupIds.length)
        throw new HttpError(
          400,
          "Canonical scenes may reference only current setup/payoff links from an explicitly merged build",
        );
    }
  }
  ...
  private normalizeInput(
    ...
    partial: boolean,
  ): CreateSceneInput & { expectedRevision?: number } {
    if (!input || typeof input !== "object" || Array.isArray(input))
      throw new HttpError(400, "Scene input is required");
    const result: CreateSceneInput & { expectedRevision?: number } = {
      ...input,
    };
    if (result.title !== undefined)
      result.title = text(result.title, "Scene title", 1_000, true);
    for (const field of ["storyDate", "storyTime"] as const) {
      const value = result[field];
      if (value !== undefined && value !== null)
        result[field] = text(value, field, 500, true);
    }
    for (const field of [
      "sceneFunction",
      "goal",
      ...
      if (result[field] !== undefined)
        result[field] = text(result[field] ?? "", field, 50_000, true);
    }
    if (result.status !== undefined && !STATUS_MAP[result.status])
      throw new HttpError(400, "Unsupported scene status");
    if (
      partial &&
      (!Number.isInteger(result.expectedRevision) ||
        (result.expectedRevision ?? -1) < 0)
    )
      throw new HttpError(
        400,
        "expectedRevision must be a non-negative integer",
    ...
    for (const field of ["povCharacterId", "locationId"] as const) {
      const value = result[field];
      if (value !== undefined && value !== null)
        result[field] = text(value, field, 500, false);
    }
    if (
      result.order !== undefined &&
      (!Number.isInteger(result.order) || result.order < 0)
    )
      throw new HttpError(400, "Scene order must be a non-negative integer");
    if (
      result.estimatedWordCount !== undefined &&
      result.estimatedWordCount !== null &&
      (!Number.isInteger(result.estimatedWordCount) ||
        result.estimatedWordCount < 0 ||
        result.estimatedWordCount > 1_000_000)
    )
      throw new HttpError(400, "Estimated word count is invalid");
    if (
      result.tension !== undefined &&
      result.tension !== null &&
      (typeof result.tension !== "number" ||
        !Number.isFinite(result.tension) ||
        result.tension < 0 ||
        result.tension > 1)
    )
      throw new HttpError(400, "Tension must be between 0 and 1");
    for (const field of [
      "characterPresentIds",
      ...
      if (result[field] !== undefined)
        result[field] = validateIds(result[field] ?? [], field);
    }
    for (const field of [
      "knowledgeDeltas",
      "objectTransfers",
      ...
      if (value !== undefined && value !== null) {
        try {
          ...
          throw new HttpError(
            400,
            error instanceof Error ? error.message : `${field} must be JSON`,
          );
        }
      }
    }
    if (
      ...
      (typeof result.content !== "string" || result.content.length > 2_000_000)
    ) {
      throw new HttpError(
        400,
        "Scene content must be a string no larger than 2 MB",
      );
    }
    return result;
  }

  private async reorderSequential(
    tx: Prisma.TransactionClient,
    chapterId: string,
    orderedIds: string[],
  ) {
    ...
    if (count !== orderedIds.length)
      throw new HttpError(409, "Scene order changed concurrently");
  }
}
...
function assignJson(
  target: Prisma.SceneUpdateInput,
  key:
    | "knowledgeDeltas"
    | "objectTransfers"
    | "injuryStateChanges"
