...
export class CompileChapterScenesUseCase {
  ...
  async execute(
    ...
    if (separator.length > 1_000)
      throw new HttpError(
        400,
        "Scene separator must be no longer than 1,000 characters",
      );
    ...
    try {
      return await this.prisma.$transaction(async (tx) => {
        ...
        if (replay) {
          if (
            replay.operation !== "compile-chapter-scenes" ||
            replay.requestHash !== requestHash
          ) {
            throw new HttpError(
              409,
              "idempotencyKey was already used for a different story operation",
            );
          }
          return {
            ...(replay.response as unknown as CompileChapterScenesResult),
            replayed: true,
          };
        }
        ...
        if (!chapter?.bodyWriting.defaultBranch)
          throw new HttpError(404, "Chapter not found");
        if (!chapter.scenes.length)
          throw new HttpError(409, "Chapter has no scenes to compile");
        ...
        if (
          ...
          throw new HttpError(
            409,
            "expectedSceneRevisions must contain every current scene exactly once; call listScenes and retry",
          );
        }
        for (const scene of chapter.scenes) {
          if (input.expectedSceneRevisions[scene.id] !== scene.revision) {
            throw new HttpError(
              409,
              `Scene '${scene.id}' revision is stale; re-read it and retry`,
              {
        ...
        const body = chapter.scenes
          .map(
            (scene) => scene.bodyWriting.defaultBranch?.headVersion?.body ?? "",
          )
          .join(separator);
        const currentBody =
          chapter.bodyWriting.defaultBranch.headVersion?.body ?? "";
        ...
        if (changed) {
          ...
          chapter.bodyWriting.defaultBranch.headVersionId !==
          input.expectedChapterHeadVersionId
        ) {
          throw new HttpError(
            409,
            "Chapter head is stale; call readChapter and retry",
          );
        }
      ...
      if (
        ...
        throw new HttpError(
          409,
          "idempotencyKey was reused concurrently for a different story operation",
        );
      }
      return {
        ...(replay.response as unknown as CompileChapterScenesResult),
...
function boundedMessage(value: string | undefined): string {
  ...
  if (normalized.length > 1_000)
    throw new HttpError(
      400,
      "Compilation message must be no longer than 1,000 characters",
    );
  return normalized;
}
