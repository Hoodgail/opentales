...
import {
  Prisma,
  type PrismaClient,
  type ProjectImport as PrismaProjectImport,
} from "@prisma/client";
import type {
...
export class ProjectImportUseCase {
  ...
  async list(
    ...
    const rows = await this.prisma.projectImport.findMany({
      where: { projectId },
  ...
  async preview(
    ...
    const existing = await this.prisma.projectImport.findUnique({
    ...
    if (existing) {
      if (existing.requestHash !== requestHash)
        throw new HttpError(
          409,
          "Import idempotency key was reused with a different file",
        );
      return toProjectImport(existing);
    }
    const conflicts = await this.chapterConflicts(
      this.prisma,
      projectId,
      parsed.chapters,
    );
    const preview = { chapters: parsed.chapters };
    const asset = await this.persistPrivateAsset(
      userId,
      projectId,
      filename,
      parsed.mimeType,
      ...
      const row = await this.prisma.projectImport.create({
        data: {
          ...
          format: parsed.format
            .toUpperCase()
            .replaceAll("-", "_") as Prisma.ProjectImportCreateInput["format"],
          filename,
  ...
  async apply(
    ...
    projectId: string,
    importId: string,
    rawInput: ApplyProjectImportInput,
  ): Promise<ProjectImportPreview> {
    await this.access.assertPermission(userId, projectId, "project:write");
    let input: z.infer<typeof applySchema>;
    try {
      input = applySchema.parse(rawInput);
    } catch (error) {
      throw new HttpError(
        400,
        error instanceof Error ? error.message : "Invalid import apply request",
      );
    }
    if (input.restoreStructuredState) {
      await this.access.assertPermission(userId, projectId, "project:admin");
      if (!input.targetBuildRunId)
        throw new HttpError(
          400,
          "targetBuildRunId is required to restore structured build state",
        );
    }
    const stored = await this.prisma.projectImport.findFirst({
      where: { id: importId, projectId },
      include: { asset: true },
    });
    if (!stored?.asset || stored.asset.isPublic)
      throw new HttpError(404, "Import preview or source file not found");
    if (stored.status === "APPLIED") {
      if (stored.applyIdempotencyKey !== input.idempotencyKey)
        throw new HttpError(
          409,
          "Import was already applied with a different idempotency key",
        );
    ...
    if (stored.expiresAt <= new Date())
      throw new HttpError(
        410,
        "Import preview expired; upload the source again",
      );
    const buffer = await this.readAsset(stored.asset.s3Key);
    if (sha256(buffer) !== stored.checksum)
      throw new HttpError(
        409,
        "Stored import checksum no longer matches its preview",
      );
    const parsed = await parseImportFile(
      buffer,
      stored.filename,
    ...
    try {
      const result = await serializableTransaction(this.prisma, async (tx) => {
        ...
        const row = await tx.projectImport.findFirst({
          where: { id: importId, projectId },
        });
        ...
        if (conflicts.length && !input.confirmConflicts)
          throw new HttpError(
            409,
            "Import conflicts require explicit confirmation",
            { conflicts },
          );
        ...
        const applied = await this.applyChapters(
          tx,
          userId,
          projectId,
          parsed.chapters,
        ...
        const structured =
          input.restoreStructuredState && input.targetBuildRunId
            ? await this.restoreStructuredState(
                tx,
                projectId,
                input.targetBuildRunId,
                parsed,
                applied,
                input.confirmConflicts,
              )
      ...
      await this.prisma.projectImport.updateMany({
        where: { id: importId, status: { not: "APPLIED" } },
        data: { status: "FAILED", error: message },
      });
      throw error;
    }
    return toProjectImport(
      await this.prisma.projectImport.findUniqueOrThrow({
        where: { id: importId },
      }),
    );
  }
  ...
  private async applyChapters(
    ...
    for (const [index, chapter] of chapters.entries()) {
      const conflict = existing.find(
        (candidate) =>
          !claimedExisting.has(candidate.id) &&
      ...
      if (conflict) {
        if (!confirmConflicts)
          throw new HttpError(
            409,
            `Chapter '${chapter.title}' conflicts with an existing chapter`,
          );
        ...
        if (!branch)
          throw new HttpError(
            409,
            `Existing chapter '${conflict.title}' has no default branch`,
          );
        const version = await tx.writingVersion.create({
          data: {
            ...
            authorId: userId,
            message: `Confirmed import from ${chapter.title}`,
          },
        });
        ...
        await tx.chapter.update({
          ...
          data: {
            title: chapter.title,
            summary: chapter.summary ?? conflict.summary,
          },
        });
        chapterId = conflict.id;
        updatedChapterIds.push(chapterId);
      } else {
        const nextNumber = await nextAvailableNumber(
          tx,
          projectId,
          chapter.number,
        );
        const writingId = await this.writings.createWriting(tx, {
          projectId,
          kind: "CHAPTER_BODY",
          body: chapter.body,
          authorId: userId,
          message: "Import chapter",
        });
      ...
      const currentScenes = await tx.scene.findMany({
        where: { chapterId },
        orderBy: { order: "asc" },
        include: { bodyWriting: { include: { defaultBranch: true } } },
      });
      for (const [sceneIndex, scene] of chapter.scenes.entries()) {
        ...
        if (target && confirmConflicts) {
          ...
          if (!branch)
            throw new HttpError(409, "Existing scene has no default branch");
          const version = await tx.writingVersion.create({
            data: {
              branchId: branch.id,
              parentVersionId: branch.headVersionId,
              body: scene.body,
              wordCount: countWords(scene.body),
              authorId: userId,
              message: "Confirmed scene import",
            },
          });
          await tx.writingBranch.update({
            where: { id: branch.id },
          ...
          await tx.scene.update({
            ...
            data: {
              title: scene.title,
              actualWordCount: countWords(scene.body),
            },
          });
          sceneId = target.id;
        } else {
          const writingId = await this.writings.createWriting(tx, {
            ...
            body: scene.body,
            authorId: userId,
            message: "Import scene",
          });
          const created = await tx.scene.create({
            data: {
              chapterId,
              order: currentScenes.length + sceneIndex,
              title: scene.title,
              status: "DRAFT",
              actualWordCount: countWords(scene.body),
              bodyWritingId: writingId,
    ...
    return {
      createdChapterIds,
      updatedChapterIds,
      chapterIdMap: Object.fromEntries(chapterIdMap),
      sceneIdMap: Object.fromEntries(sceneIdMap),
    };
  }
  ...
  private async restoreStructuredState(
    ...
    const run = await tx.buildRun.findFirst({
    ...
    if (!scope.allowPlanningArtifacts || !scope.allowCanonWrites)
      throw new HttpError(
        403,
        "Target build is not authorized for artifact and canon restoration",
      );
    ...
    for (const raw of arrayValue(archiveBuild.artifacts)) {
      ...
      const type = stringValue(source.type)
        ?.toLowerCase()
        .replaceAll("_", "-") as
        | Parameters<typeof validateArtifactContent>[0]
        | undefined;
      ...
      const current = await tx.storyArtifact.findFirst({
        where: {
          buildRunId,
          type: type.toUpperCase().replaceAll("-", "_") as never,
          key,
          invalidatedAt: null,
      ...
      if (current && !confirmConflicts)
        throw new HttpError(
          409,
          `Structured artifact '${type}/${key}' already exists`,
        );
      if (current)
        await tx.storyArtifact.update({
          where: { id: current.id },
          data: { status: "SUPERSEDED" },
        });
      const created = await tx.storyArtifact.create({
        data: {
          projectId,
          buildRunId,
          type: type.toUpperCase().replaceAll("-", "_") as never,
          key,
          title: stringValue(source.title) ?? key,
          version: (current?.version ?? 0) + 1,
          schemaVersion: stringValue(source.schemaVersion) ?? "story-ir-v1",
          status: "VALIDATED",
          content: content as Prisma.InputJsonValue,
          contentHash: stableHash(content),
          replacesArtifactId: current?.id ?? null,
        },
      });
    ...
    for (const raw of arrayValue(archiveBuild.canonFacts)) {
      ...
      if (
        ...
        continue;
      const current = await tx.canonFact.findFirst({
        where: { buildRunId, key, isCurrent: true },
        orderBy: { version: "desc" },
      });
      if (current && !confirmConflicts)
        throw new HttpError(409, `Canon fact '${key}' already exists`);
      if (current)
      ...
      const fact = await tx.canonFact.create({
        data: {
          ...
          subjectId,
          predicate,
          object: jsonInput(source.object),
          version: (current?.version ?? 0) + 1,
          isCurrent: true,
          supersedesFactId: current?.id ?? null,
          status: canonStatus(source.status),
          confidence: numberValue(source.confidence, 1),
          sourceArtifactId: mapped(oldToNewArtifact, source.sourceArtifactId),
          sourceChapterId: mappedRecord(
            applied.chapterIdMap,
            source.sourceChapterId,
          ),
          sourceSceneId: mappedRecord(applied.sceneIdMap, source.sourceSceneId),
          validFromSceneId: mappedRecord(
            applied.sceneIdMap,
            source.validFromSceneId,
          ),
          validToSceneId: mappedRecord(
            applied.sceneIdMap,
            source.validToSceneId,
          ),
          sourceSpan:
            source.sourceSpan === null || source.sourceSpan === undefined
              ? Prisma.JsonNull
              : jsonInput(source.sourceSpan),
          validFromOrder:
            typeof source.validFromOrder === "number"
              ? source.validFromOrder
              : null,
          validToOrder:
            typeof source.validToOrder === "number"
              ? source.validToOrder
              : null,
        },
      });
  ...
  private async chapterConflicts(
    client: PrismaClient | Prisma.TransactionClient,
    projectId: string,
    chapters: ImportPreviewChapter[],
  ): Promise<ProjectImportConflict[]> {
    const existing = await client.chapter.findMany({
      where: { projectId, deletedAt: null },
    ...
    for (const chapter of chapters) {
      ...
      if (number)
        conflicts.push({
          kind: "chapter-number",
          sourceKey: String(chapter.number),
          existingId: number.id,
          message: `Chapter number ${chapter.number} already belongs to '${number.title}'.`,
        });
      const title = existing.find(
        (item) => item.title.toLowerCase() === chapter.title.toLowerCase(),
      );
      if (title && title.id !== number?.id)
        conflicts.push({
          kind: "chapter-title",
          sourceKey: chapter.title,
          existingId: title.id,
          message: `Chapter title '${chapter.title}' already exists.`,
        });
    }
    return conflicts;
  }

  private async persistPrivateAsset(
    userId: string,
    projectId: string,
    filename: string,
    mimeType: string,
    ...
    const asset = await this.prisma.asset.create({
      data: {
        ...
        s3Key: `pending/import/${randomUUID()}`,
        mimeType,
    ...
    try {
      const stored = await this.storage.write(
        projectId,
        asset.id,
        path.extname(filename).slice(1) || "bin",
        buffer,
      );
      return await this.prisma.asset.update({
        where: { id: asset.id },
        data: { s3Key: stored.key, sizeBytes: stored.sizeBytes },
      });
    } catch (error) {
      await this.prisma.asset.deleteMany({ where: { id: asset.id } });
      throw error;
  ...
  private async readAsset(key: string): Promise<Buffer> {
    const chunks: Buffer[] = [];
    for await (const chunk of await this.storage.readStream(key))
  ...
  for (let attempt = 0; attempt < 12; attempt += 1)
    try {
      ...
      await new Promise((resolve) =>
        setTimeout(
          resolve,
          Math.min(500, 20 * 2 ** Math.min(attempt, 4)) +
...
function isSerializationConflict(error: unknown): boolean {
  if (
    ...
    return true;
  return /40001|40P01|serialize access|serialization failure|deadlock/i.test(
...
function toProjectImport(row: PrismaProjectImport): ProjectImportPreview {
  ...
  return {
    ...
    format: row.format
      .toLowerCase()
      .replaceAll("_", "-") as ProjectImportPreview["format"],
    status: row.status.toLowerCase() as ProjectImportPreview["status"],
    filename: row.filename,
    mimeType: row.mimeType,
    checksum: row.checksum,
    sizeBytes: Number(row.sizeBytes),
    chapters: Array.isArray(preview.chapters)
      ? (preview.chapters as unknown as ImportPreviewChapter[])
      : [],
    conflicts: Array.isArray(row.conflicts)
      ? (row.conflicts as unknown as ProjectImportConflict[])
      : [],
    sourceMetadata: row.sourceMetadata as JsonValue,
    expiresAt: row.expiresAt.toISOString(),
    appliedAt: row.appliedAt?.toISOString() ?? null,
    applyResult: row.applyResult as JsonValue | null,
    error: row.error,
...
async function nextAvailableNumber(
  ...
  while (await tx.chapter.count({ where: { projectId, number: value } }))
    value += 1;
  return value;
}
...
function safeUploadName(value: string): string {
  ...
  if (!name || name.length > 255)
    throw new HttpError(400, "Import filename is invalid");
  return name;
}
...
function canonStatus(value: unknown): CanonStatusValue {
  ...
  return [
    ...
    ? (normalized as CanonStatusValue)
    : "CANONICAL";
}
