...
export class ManuscriptSnapshotService {
  ...
  async main(
    ...
    const chapters: ManuscriptChapterSnapshot[] = project.chapters.map(
      (chapter) => {
        const branch = chapter.bodyWriting.defaultBranch;
        if (!branch?.headVersion)
          throw new HttpError(
            409,
            `Chapter ${chapter.number} has no main writing version`,
          );
        const chapterBody = branch.headVersion.body ?? "";
        const sceneBody = chapter.scenes
          .map((scene) => bodyOf(scene.bodyWriting))
    ...
    if (!chapters.length)
      throw new HttpError(409, "Project has no chapters to export");
    return snapshot({
      projectId,
  ...
  async build(
    ...
    const chapters = compilation.units
      ...
      .sort(
        (a, b) =>
          (a.unit.chapterNumber ?? a.unit.order) -
          (b.unit.chapterNumber ?? b.unit.order),
      )
      .map((row): ManuscriptChapterSnapshot => {
        const body = row.writingVersion.body ?? "";
        if (stableHash(body) !== row.contentHash)
          throw new HttpError(
            409,
            `Compilation content hash drifted for unit '${row.unit.key}'`,
          );
        return {
    ...
    if (!chapters.length)
      throw new HttpError(409, "Compilation has no chapter units");
  ...
  async projectArchive(
    ...
    if (!project) throw new HttpError(404, "Project not found");
    const build = snapshotValue.buildRunId
      ? await this.buildArchiveState(
          snapshotValue.projectId,
          snapshotValue.buildRunId,
        )
      : null;
    ...
    for (const asset of project.assets) {
      const archivePath = safeArchivePath(
        `assets/${asset.id}${path.extname(asset.s3Key) || ""}`,
      );
      let included = false;
      if (includeAssets && asset.s3Bucket === LocalAssetStorage.bucket) {
        const chunks: Buffer[] = [];
        for await (const chunk of await this.storage.readStream(asset.s3Key))
        ...
        if (totalAssetBytes > MAX_ARCHIVE_ASSET_BYTES)
          throw new HttpError(
            413,
            "Project assets exceed the 100 MB archive limit",
          );
    ...
    const payload: ProjectArchivePayload = {
      ...
      source: {
        ...
        buildRunId: snapshotValue.buildRunId,
      },
      project: jsonRecord({
        title: project.title,
        slug: project.slug,
      ...
      chapters: project.chapters.map((chapter) =>
        jsonRecord({
          ...
          scenes: chapter.scenes.map((scene) => ({
            ...serializable(scene, ["bodyWriting"]),
            body: bodyOf(scene.bodyWriting),
          })),
        }),
      ),
      ...
      relationships: project.characterRelationships.map((relationship) =>
        jsonRecord(serializable(relationship)),
      ),
      docs: project.docs.map((doc) =>
        jsonRecord({
          ...serializable(doc, ["bodyWriting"]),
          body: bodyOf(doc.bodyWriting),
        }),
      ),
      storyStructure: project.storyStructure
        ? jsonRecord({
            ...serializable(project.storyStructure, [
              "loglineWriting",
              "outlineWriting",
              "climaxWriting",
            ]),
            logline: bodyOf(project.storyStructure.loglineWriting),
            outline: bodyOf(project.storyStructure.outlineWriting),
            climax: bodyOf(project.storyStructure.climaxWriting),
          })
        : null,
      build,
      assets,
  ...
  private async buildArchiveState(
    projectId: string,
    buildRunId: string,
    ...
    return jsonRecord({
      ...
      artifacts: artifacts.map((item) => serializable(item)),
      canonFacts: facts.map((item) => serializable(item)),
      entityStates: states.map((item) => serializable(item)),
      timelineEvents: events.map((item) => serializable(item)),
      openLoops: loops.map((item) => serializable(item)),
      setupPayoffs: setups.map((item) => serializable(item)),
      plotThreads: threads.map((item) => serializable(item)),
      manuscriptUnits: units.map((unit) => ({
        ...serializable(unit, ["branch"]),
...
function snapshot(
  input: Omit<
    ManuscriptSnapshot,
    "contentHash" | "totalWordCount" | "branchHeads"
  >,
): ManuscriptSnapshot {
  const branchHeads = input.chapters.map((chapter) => ({
    sourceId: chapter.sourceId,
    writingId: chapter.writingId,
  ...
  return {
    ...input,
    totalWordCount: input.chapters.reduce(
...
function serializable<T extends object>(
  value: T,
  omit: string[] = [],
