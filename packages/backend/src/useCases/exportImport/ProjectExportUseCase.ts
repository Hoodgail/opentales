...
import {
  Prisma,
  type PrismaClient,
  type ProjectExport as PrismaProjectExport,
} from "@prisma/client";
import type {
    ...
    target: z.discriminatedUnion("kind", [
      ...
      z
        .object({
          ...
          compilationId: z.string().trim().min(1).nullable().optional(),
        })
        .strict(),
    ]),
    options: z
...
export class ProjectExportUseCase {
  ...
  async create(
    ...
    try {
      input = createSchema.parse(rawInput);
    } catch (error) {
      throw new HttpError(
        400,
    ...
    if (prior) {
      if (prior.requestHash !== requestHash)
        throw new HttpError(
          409,
          "Export idempotency key was reused with different input",
        );
    ...
    try {
      row = await this.prisma.projectExport.create({
        data: {
          projectId,
          buildRunId:
            input.target.kind === "build" ? input.target.buildRunId : null,
          compilationId:
            input.target.kind === "build"
              ? (input.target.compilationId ?? null)
              : null,
      ...
      const replay = await this.prisma.projectExport.findUnique({
        where: {
          projectId_idempotencyKey: {
            projectId,
            idempotencyKey: input.idempotencyKey,
    ...
    try {
      ...
      if (asset)
        await this.deleteAssetBytesAndRow(asset).catch(() => undefined);
      const message =
        error instanceof Error ? error.message : "Export generation failed";
      row = await this.prisma.projectExport.update({
        where: { id: row.id },
        data: { assetId: null, status: "FAILED", error: message },
      });
  ...
  async regenerate(
    ...
    if (existing.status === "DELETED")
      throw new HttpError(409, "Deleted exports cannot be regenerated");
    ...
    return this.create(
      ...
      projectId,
      {
        ...
        preset: existing.preset
          .toLowerCase()
          .replaceAll("_", "-") as CreateProjectExportInput["preset"],
        target:
          existing.target === "BUILD"
            ? {
                kind: "build",
                buildRunId: existing.buildRunId!,
                compilationId: existing.compilationId,
              }
            : { kind: "main" },
        options,
      },
  ...
  async delete(
    ...
    await this.access.assertPermission(userId, projectId, "project:write");
    const existing = await this.prisma.projectExport.findFirst({
    ...
    const updated = await this.prisma.$transaction(async (tx) => {
      const locked = await tx.projectExport.findFirst({
        where: { id: exportId, projectId },
      });
      if (!locked) throw new HttpError(404, "Export not found");
      if (locked.assetId) {
        const asset = await tx.asset.findUnique({
          where: { id: locked.assetId },
        });
        key = asset?.s3Key ?? null;
        const manifests = await tx.storyArtifact.findMany({
          where: { projectId, type: "EXPORT_MANIFEST", invalidatedAt: null },
        });
        for (const manifest of manifests) {
          const outputs = Array.isArray(objectValue(manifest.content).outputs)
            ? (objectValue(manifest.content).outputs as unknown[])
            : [];
          if (
            !outputs.some(
              (output) => objectValue(output).assetId === locked.assetId,
            )
          )
            continue;
          ...
          await tx.buildCompilation.updateMany({
            where: { exportManifestArtifactId: manifest.id },
            data: { exportManifestArtifactId: null },
          });
        }
        await tx.asset.delete({ where: { id: locked.assetId } });
      }
      return tx.projectExport.update({
        where: { id: exportId },
        data: { assetId: null, status: "DELETED", deletedAt: new Date() },
      });
    });
  ...
  async download(userId: string, projectId: string, exportId: string) {
    ...
    if (!row?.asset || row.asset.isPublic)
      throw new HttpError(404, "Export file not found");
    if (row.asset.s3Bucket !== LocalAssetStorage.bucket)
      throw new HttpError(501, "Export is stored in an unsupported bucket");
    return {
  ...
  private async resolveSnapshot(
    ...
    if (input.target.kind === "main")
      return this.snapshots.main(projectId, input.options.authorName);
    const compilationId = input.target.compilationId;
    if (!compilationId)
      throw new HttpError(
        410,
        "Workflow compilation has been retired; export the main manuscript or an existing historical compilation",
      );
    return this.snapshots.build(
      projectId,
      input.target.buildRunId,
      compilationId,
      input.options.authorName,
    );
  }
  ...
  private async generate(
    ...
    const archive = await this.snapshots.projectArchive(
      snapshot,
...
function toProjectExport(row: PrismaProjectExport): ProjectExport {
  return {
    ...
    preset: row.preset
      .toLowerCase()
      .replaceAll("_", "-") as ProjectExport["preset"],
    ...
    error: row.error,
    downloadUrl:
      row.status === "READY" && !row.deletedAt
        ? `${env.publicBaseUrl}/projects/${row.projectId}/exports/${row.id}/download`
        : null,
...
function toPrismaFormat(format: ProjectExportFormat) {
  return format.toUpperCase().replaceAll("-", "_") as
    | "DOCX"
    | "PDF"
    | "EPUB"
    | "MARKDOWN"
...
function fromPrismaFormat(
  format: PrismaProjectExport["format"],
): ProjectExportFormat {
...
function toPrismaPreset(preset: CreateProjectExportInput["preset"]) {
  return preset.toUpperCase().replaceAll("-", "_") as
    | "STANDARD_MANUSCRIPT"
    | "READING_COPY"
    | "EBOOK"
    | "WEB"
...
function validateSnapshotBounds(snapshot: ManuscriptSnapshot): void {
  if (snapshot.chapters.length > 2_000)
    throw new HttpError(413, "Exports are limited to 2,000 chapters");
  const bytes = snapshot.chapters.reduce(
    (sum, chapter) => sum + Buffer.byteLength(chapter.body, "utf8"),
    0,
  );
  if (bytes > 50 * 1024 * 1024)
    throw new HttpError(
      413,
      "Manuscript prose exceeds the 50 MB export input limit",
    );
}
