...
describe("AiAgentSessionUseCase stream persistence", () => {
  it("requires project-admin authority to enable Auto mode and persists it on an idle session", async () => {
    ...
    useCase.snapshot = vi.fn(async () => ({
      ...sessionSnapshot("session-mode"),
      approvalMode: "auto",
    }));
    useCase.broadcast = vi.fn(async () => undefined);

    const result = await useCase.update("user-1", "project-1", "session-mode", {
      approvalMode: "auto",
    });
  ...
  it("executes mutation tools immediately in Auto mode without creating an approval waiter", async () => {
    const tool = row({
      ...
      promptId: "auto-prompt",
      toolCallId: "provider-auto",
    ...
    useCase.ensureToolTimelinePart = vi.fn(async () =>
      storedPart({
        id: "auto-call-part",
        sessionId: "auto-session",
        promptId: "auto-prompt",
    ...
    await expect(
      useCase.handleApproval(
        "auto-session",
        "auto-prompt",
        "project-1",
        "createChapter",
        { title: "Immediate chapter" },
        execute,
        "provider-auto",
        undefined,
        "auto",
  ...
  it("bounds snapshot tool output while preserving an explicit full-detail projection", () => {
    ...
    expect(projectToolOutput(value, false)).toMatchObject({
      truncated: true,
      bytes: expect.any(Number),
    });
    expect(projectToolOutput(value, true)).toEqual({
      output: value,
      truncated: false,
      bytes: expect.any(Number),
    });
    expect(projectToolInput(value, false)).toMatchObject({
      truncated: true,
      bytes: expect.any(Number),
    });
    expect(projectToolInput(value, true)).toEqual({
      value,
      truncated: false,
      bytes: expect.any(Number),
    });
    const pending = toToolCall(
      row({ input: value, status: "PENDING_APPROVAL" }) as any,
    );
  ...
  it("pages durable timeline parts strictly before a sequence cursor", async () => {
    const parts = [9, 8, 7].map((sequence) =>
      storedPart({
        id: `page-${sequence}`,
        sequence,
        kind: "task",
        payload: {
          sessionId: `child-${sequence}`,
    ...
    expect(
      page.parts.map((part: { sequence: number }) => part.sequence),
    ).toEqual([8, 9]);
  ...
  it("advances through more than 200 pure-legacy records with a non-null opaque cursor", async () => {
        ...
        return rows
          .filter(
            (row) =>
              !date ||
              row.createdAt < date ||
              (row.createdAt.getTime() === date.getTime() &&
                Boolean(tieId) &&
                row.id < tieId!),
          )
          .slice(0, take);
    ...
    let pages = 0;
    do {
      const page = await useCase.legacyTimelinePage(
        "legacy-session",
        beforeSequence,
        10,
        cursor,
      ...
      if (page.parts.length) {
        ...
        ).toBeLessThan(previousEarliest);
        previousEarliest = Math.min(
          ...page.parts.map((part: { sequence: number }) => part.sequence),
        );
      }
  ...
  it("keeps a legacy executed tool call/result indivisible when limit is one", async () => {
    ...
    expect(page.parts.map((part: { kind: string }) => part.kind)).toEqual([
      "tool-call",
      "tool-result",
    ]);
  ...
  it("updates a reused provider toolCallId only inside the originating session", async () => {
    ...
    expect(rows.find((item) => item.id === "call-b")).toMatchObject({
      status: "RUNNING",
      output: null,
    });
    expect(prisma.aiAgentToolCall.findFirst).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it("delegates document work without inheriting historical builds and emits task lifecycle events", async () => {
    ...
    const prisma = {
      ...
      aiAgentMessage: {
        findFirst: vi.fn(async () => ({ content: "Subtask complete." })),
      },
    };
    const useCase = new AiAgentSessionUseCase(
      prisma as unknown as PrismaClient,
    ) as any;
    useCase.getSession = vi.fn(async () => ({
      id: "child-session",
      projectId: "project-1",
    }));
    useCase.drain = vi.fn(async () => undefined);
    useCase.snapshot = vi.fn(async (sessionId: string) =>
      sessionSnapshot(sessionId),
    );
    useCase.broadcast = vi.fn(
      async (_projectId: string, event: { type: string; data?: unknown }) => {
        broadcasts.push(event);
      },
    );
    useCase.createProviderToolCall = vi.fn(async () =>
      row({
        id: "parent-task-call",
        sessionId: "parent-session",
        promptId: "parent-prompt",
        toolCallId: "provider-task-1",
        toolName: "task",
        status: "RUNNING",
      }),
    );
    useCase.ensureToolTimelinePart = vi.fn(
      async (_sessionId: string, promptId: string, kind: string) =>
        storedPart({
          id: `${kind}-part`,
          sequence: kind === "tool-call" ? 1 : 4,
          promptId,
          kind,
          toolCallId: "parent-task-call",
        }),
    );
    useCase.finalizeHandlerToolCall = vi.fn(async () => {
      broadcasts.push({
        type: "tool-result",
        data: { toolCallId: "provider-task-1" },
      });
    });
    let sequence = 0;
    useCase.createTaskPart = vi.fn(
      async (
        _sessionId: string,
        promptId: string,
    ...
    const result = await useCase.handleTask(
      ...
      "user-1",
      {
        description: "Initialize planning",
        objective: "Plan the novel",
    ...
    expect(
      broadcasts.filter((event) => event.type === "tool-call"),
    ).toHaveLength(1);
    expect(
      broadcasts.filter((event) => event.type === "tool-result"),
    ).toHaveLength(1);
    ...
    expect(shouldBroadcastOuterToolEvent("task", "tool-input-error")).toBe(
      true,
    );
    expect(shouldBroadcastOuterToolEvent("task", "tool-call", false)).toBe(
      true,
    );
    expect(
      (broadcasts[1]?.data as { part: { task: { status: string } } }).part.task
        .status,
    ).toBe("running");
    expect(
      (broadcasts[2]?.data as { part: { task: { status: string } } }).part.task
        .status,
    ).toBe("completed");
    const persistedPayload = JSON.parse(
      promptCreate.mock.calls[0]![0].data.prompt as string,
    ) as {
      buildRunId: string;
      approvalMode: string;
      taskContract: { scope: { buildRunId: string } };
    };
    expect(persistedPayload.buildRunId).toBeNull();
    expect(persistedPayload.approvalMode).toBe("auto");
    expect(persistedPayload.taskContract.scope.buildRunId).toBeUndefined();
    expect(childSessionUpdate).toHaveBeenCalledWith(
      expect.objectContaining({
        data: { activeBuildRunId: null, approvalMode: "AUTO" },
      }),
    );
  });
  ...
  it("uses an atomic approval CAS so concurrent approvals execute exactly once", async () => {
    ...
    const prisma = {
      aiAgentToolCall: {
        ...
        update: vi.fn(async ({ data }: { data: Partial<ToolRow> }) =>
          Object.assign(tool, data),
        ),
    ...
    useCase.ensureToolTimelinePart = vi.fn(
      async (_sessionId: string, promptId: string, kind: string) =>
        storedPart({ id: `${kind}-cas`, promptId, kind, toolCallId: tool.id }),
    );
    useCase.createMessagePart = vi.fn(async () =>
      storedPart({ id: "message-cas", kind: "message" }),
    );
    useCase.hydrateSinglePart = vi.fn(async () => null);
  ...
  it("marks an orphan RUNNING prompt actionable after restart instead of wedging the queue", async () => {
    ...
    const snapshot = await useCase.get(
      "user-restart",
      "project-restart",
      "restart-session",
    );
  ...
  it("records a cancelled child as cancelled and never reports partial output completed", async () => {
    ...
    const prisma = {
      ...
      aiAgentMessage: {
        findFirst: vi.fn(async () => ({ content: "Partial, not complete." })),
      },
    };
    const useCase = new AiAgentSessionUseCase(
      prisma as unknown as PrismaClient,
    ...
    useCase.createProviderToolCall = vi.fn(async () =>
      row({
        id: "cancelled-call",
        sessionId: "parent-cancelled",
        promptId: "parent-prompt",
        toolCallId: "provider-cancelled",
        toolName: "task",
        status: "RUNNING",
      }),
    );
    useCase.ensureToolTimelinePart = vi.fn(
      async (_sessionId: string, promptId: string, kind: string) =>
        storedPart({ id: `${kind}-cancelled`, promptId, kind }),
    );
    useCase.createTaskPart = vi.fn(
      async (
        _sessionId: string,
    ...
    expect(lifecycle.map((task) => task.status)).toEqual([
      "running",
      "cancelled",
    ]);
    expect(useCase.finalizeHandlerToolCall).toHaveBeenCalledWith(
  ...
  it("records an already-aborted parent task as cancelled after listener registration", async () => {
    ...
    useCase.createProviderToolCall = vi.fn(async () =>
      row({
        id: "abort-call",
        sessionId: "abort-parent",
        promptId: "abort-prompt",
        toolCallId: "provider-abort",
        toolName: "task",
        status: "RUNNING",
      }),
    );
    useCase.ensureToolTimelinePart = vi.fn(
      async (_sessionId: string, promptId: string, kind: string) =>
        storedPart({ id: `${kind}-abort`, promptId, kind }),
    );
    useCase.createTaskPart = vi.fn(
      async (
        _sessionId: string,
        ...
        lifecycle.push(task);
        return storedPart({
          id: `abort-task-${lifecycle.length}`,
          promptId,
          kind: "task",
          payload: JSON.parse(JSON.stringify(task)),
        });
    ...
    useCase.broadcast = vi.fn(async () => undefined);
    useCase.finalizeHandlerToolCall = vi.fn(async () => undefined);
    useCase.drain = vi.fn(async () => {
      throw new Error("drain must not start");
    });
  ...
  it("does not hang approval or question handlers when abort happened before listener registration", async () => {
    ...
    const useCase = new AiAgentSessionUseCase({} as PrismaClient) as any;
    useCase.createProviderToolCall = vi.fn(
      async (_session: string, _prompt: string, id: string) =>
        id === "provider-question" ? question : approval,
    );
    useCase.ensureToolTimelinePart = vi.fn(
      async (_sessionId: string, promptId: string, kind: string) =>
        storedPart({ id: `${kind}-preabort`, promptId, kind }),
    );
    useCase.hydrateSinglePart = vi.fn(async () => null);
  ...
  it("replays an idempotent approved writing mutation after restart but fails unsafe CRUD closed", async () => {
    const safe = row({
      id: "safe-approved",
      toolName: "applyStoryPatch",
      status: "APPROVED",
    });
    const unsafe = row({
      id: "unsafe-pending",
    ...
    await useCase.applyToolCallApproval(
      "user",
      "project",
      "session-a",
      safe,
    ...
    expect(useCase.recoverApprovedToolCall).toHaveBeenCalledWith(
      "user",
      "project",
      "session-a",
      safe,
    ...
    expect(useCase.failOrphanedInteractiveRun).toHaveBeenCalledWith(
      "session-a",
      unsafe.promptId,
      expect.stringMatching(/cannot be replayed safely/),
      unsafe.id,
    );
  });
  ...
  it("requires complete question answers and unwedge-finalizes restart fallbacks", async () => {
    ...
    useCase.createMessagePart = vi.fn(async () =>
      storedPart({ id: "question-message-part", kind: "message" }),
    );
    useCase.hydrateSinglePart = vi.fn(async () => null);
    ...
    expect(useCase.failOrphanedInteractiveRun).toHaveBeenCalledWith(
      "question-session",
      question.promptId,
      expect.stringMatching(/cannot continue/),
    );
  });
  ...
  it("buffers live deltas until the initial SSE snapshot is written", async () => {
    ...
    const snapshotPromise = new Promise<ReturnType<typeof sessionSnapshot>>(
      (resolve) => {
    ...
    const subscribing = useCase.subscribe(
      "user",
      "project",
      res,
      "race-session",
    ...
    await useCase.broadcast(
      "project",
      { type: "text-delta", data: { text: "newer" } },
      "race-session",
    );
  ...
  it("waits for drain after initial SSE backpressure and retires throwing clients", async () => {
    ...
    await useCase.broadcast(
      "project",
      { type: "text-delta", data: { text: "buffered" } },
      "pressure-session",
    );
    ...
    await expect(
      useCase.subscribe("user", "project", throwing, "throwing-session"),
    ).resolves.toBeUndefined();
    await expect(
      useCase.broadcast(
        "project",
        { type: "text-delta", data: { text: "ignored" } },
        "throwing-session",
      ),
    ).resolves.toBeUndefined();
  ...
  it("serializes per-session drains and database-claims a queued prompt once", async () => {
    ...
    const tx = {
      projectAiAgentSession: {
        updateMany: vi.fn(async () => {
          if (sessionStatus === "RUNNING" || activePromptId !== null)
            return { count: 0 };
    ...
    expect(
      await claimUseCase.claimQueuedPrompt("mutex-session", "prompt-1"),
    ).toBe(true);
    expect(
      await claimUseCase.claimQueuedPrompt("mutex-session", "prompt-1"),
    ).toBe(false);
    ...
    expect(
      tx.aiAgentPrompt.updateMany.mock.invocationCallOrder[0],
    ).toBeLessThan(
  ...
  it("installs cancellation before the first drain lookup and never claims work after a concurrent Stop", async () => {
    ...
    const sessionLookup = new Promise<{ id: string; projectId: string }>(
      (resolve) => {
        resolveSession = resolve;
      },
    );
    const findQueued = vi.fn(async () => ({
      id: "must-not-start",
      prompt: "Do work",
    }));
    ...
    useCase.snapshot = vi.fn(async () =>
      sessionSnapshot("cancel-race-session"),
    );
    useCase.broadcast = vi.fn(async () => undefined);
    useCase.drain = vi.fn(async () => undefined);

    const draining = useCase.drainOwned(
      "user",
      "project",
      "cancel-race-session",
    );
  ...
  it("refuses to overwrite cancellation when terminal completion loses its conditional session claim", async () => {
    ...
    await expect(
      useCase.completeClaimedPrompt(
        "completion-race-session",
        "completion-race-prompt",
      ),
    ).resolves.toBe(false);
    expect(promptUpdate).toHaveBeenCalledWith({
      where: {
...
function toolPrisma(rows: ToolRow[]) {
  return {
    aiAgentToolCall: {
      findFirst: vi.fn(
        async ({ where }: { where: Partial<ToolRow> }) =>
          rows.find((item) =>
            Object.entries(where).every(
              ([key, value]) => item[key as keyof ToolRow] === value,
            ),
          ) ?? null,
      ),
