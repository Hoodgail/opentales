...
const HANDLER_MANAGED_TOOLS = new Set<string>([
  ...agentMutatingToolNames,
  "askUser",
  "task",
]);
...
const APPROVAL_TIMEOUT_MS = Number(
  process.env.AI_APPROVAL_TIMEOUT_MS ?? 10 * 60 * 1000,
);
const QUESTION_TIMEOUT_MS = Number(
  process.env.AI_QUESTION_TIMEOUT_MS ?? APPROVAL_TIMEOUT_MS,
);
const DEFAULT_CONTEXT_WINDOW = Number(
  process.env.AI_CONTEXT_WINDOW_TOKENS ?? 128_000,
);
const TOOL_OUTPUT_SNAPSHOT_BYTES = Math.max(
  1_024,
  Number(process.env.AI_TOOL_OUTPUT_SNAPSHOT_BYTES ?? 16_384),
);
...
export class AiAgentSessionUseCase {
  ...
  async update(
    ...
    if (session.status === "RUNNING")
      throw new HttpError(
        409,
        "Execution mode cannot change while the agent is running",
      );
    ...
    if (queued > 0)
      throw new HttpError(
        409,
        "Execution mode cannot change while prompts are queued",
      );
  ...
  async getTimeline(
    ...
    const beforeSequence =
      typeof input.beforeSequence === "number" &&
      Number.isInteger(input.beforeSequence)
    ...
    if (selected.length) {
      const messageIds = [
        ...new Set(
          selected.flatMap((part) => (part.messageId ? [part.messageId] : [])),
        ),
      ];
      const toolCallIds = [
        ...new Set(
      ...
      const [messages, toolCalls, legacyMessage, legacyTool] =
        await Promise.all([
          messageIds.length
            ? this.prisma.aiAgentMessage.findMany({
                where: { sessionId: session.id, id: { in: messageIds } },
              })
            : [],
          toolCallIds.length
            ? this.prisma.aiAgentToolCall.findMany({
                where: { sessionId: session.id, id: { in: toolCallIds } },
              })
            : [],
          rows.length <= limit
            ? this.prisma.aiAgentMessage.findFirst({
                where: { sessionId: session.id, parts: { none: {} } },
                select: { id: true },
              })
            : null,
          rows.length <= limit
            ? this.prisma.aiAgentToolCall.findFirst({
                where: { sessionId: session.id, parts: { none: {} } },
                select: { id: true },
              })
            : null,
      ...
      const hasMore =
        rows.length > limit || Boolean(legacyMessage || legacyTool);
      ...
      return {
        ...
        nextBeforeSequence: hasMore
          ? Math.min(...selected.map((part) => part.sequence))
          : null,
        hasMore,
      };
    }
    return this.legacyTimelinePage(
      session.id,
      beforeSequence,
      limit,
      input.legacyCursor,
  ...
  async approveToolCall(
          ...
          { OR: [{ id: toolCallId }, { toolCallId }] },
          {
            OR: [
              { status: "PENDING_APPROVAL" },
              {
                ...
                toolName: { in: [...agentMutatingToolNames] },
              },
            ],
          },
        ],
    ...
    await this.applyToolCallApproval(
      userId,
      projectId,
      session.id,
      toolCall,
  ...
  async approveToolCalls(
    ...
    const ids = [
      ...new Set(
        Array.isArray(input.toolCallIds)
          ? input.toolCallIds.filter(Boolean)
          : [],
      ),
    ];
    ...
    if (toolCalls.length !== ids.length)
      throw new HttpError(404, "One or more pending tool calls were not found");
  ...
  async answerQuestion(
    ...
    if (claimed.count !== 1)
      throw new HttpError(409, "Question was already answered or dismissed");
    const answeredToolCall =
      await this.prisma.aiAgentToolCall.findUniqueOrThrow({
  ...
  async subscribe(
    ...
    if (client.closed) return;
    const initialWritten = safeWriteRuntimeEvent(client, {
      type: "session",
      session: initial,
    });
  ...
  private async drainControlled(
    ...
    while (true) {
      if (runtime.cancelPending || controllerState.current.signal.aborted)
        return;
      ...
      const flushText = async (force: boolean): Promise<void> => {
        ...
        if (
          ...
          (Date.now() - lastTextFlushAt < 250 || pendingText.length < 8)
        )
          return;
        ...
        activeTextPart = activeTextPart
          ? await this.updateTextPart(
              activeTextPart,
              readPartText(activeTextPart) + delta,
              true,
            )
          : await this.createTextPart(
              session.id,
              queued.id,
              assistantMessage.id,
              delta,
        ...
        await this.broadcast(
          projectId,
          {
            ...
            data: {
              promptId: queued.id,
              text: delta,
              part: toTextTimelinePart(activeTextPart),
            },
          },
          session.id,
        );
      ...
      try {
        if (controller.signal.aborted) {
          throw new AgentStreamAbort(
            "Agent prompt was cancelled before execution",
          );
        }
        const agents = await loadAiAgents(this.prisma, projectId);
        const activeAgent = promptPayload.agent
          ? findAgent(agents, promptPayload.agent)
          : undefined;
        const selectedModelId = activeAgent?.model ?? promptPayload.model;
        const model = await loadAiModelForProject(
          this.prisma,
          projectId,
          selectedModelId,
        );
        const { systemPrompt, userPrompt, skillAllowedTools } =
        ...
        const messagePrompt = modelContent(
          userPrompt,
          promptPayload.attachments,
        );
        const messages = [
          { role: "user" as const, content: messagePrompt },
        ...
        for await (const part of result.fullStream) {
          ...
          if (terminal?.kind === "abort")
            throw new AgentStreamAbort(terminal.message);
          if (terminal?.kind === "error")
            throw new AgentStreamFailure(terminal.message);
          if (type === "text-delta") {
            ...
            const handlerOwned = Boolean(
              parsedTool &&
                isHandlerManagedTool(parsedTool.toolName) &&
                !invalidToolCall,
            );
            if (
              parsedTool &&
              ...
              continue;
            const persisted = await this.persistToolCall(
              session.id,
              queued.id,
              part,
            );
            if (
              parsedTool?.toolCallId &&
              isHandlerManagedTool(parsedTool.toolName) &&
              !handlerOwned
            ) {
            ...
            const timelinePart = persisted
              ? await this.ensureToolTimelinePart(
                  session.id,
                  queued.id,
                  "tool-call",
                  persisted.id,
            ...
            await this.broadcast(projectId, {
              ...
              data: mergeEventPart(
                persisted ?? part,
                timelinePart
                  ? await this.hydrateSinglePart(timelinePart)
                  : null,
              ),
            });
          } else if (
            ...
            let handlerCall = parsedResult?.toolCallId
              ...
              : null;
            const eventToolName =
              stringValue(objectRecord(part)?.toolName) ??
              handlerCall?.toolName ??
              null;
            const handlerOwned =
              Boolean(handlerCall) &&
            ...
            if (
              eventToolName &&
              !shouldBroadcastOuterToolEvent(eventToolName, type, handlerOwned)
            )
              continue;
            ...
            if (parsedResult?.toolCallId)
              outerOwnedToolCalls.delete(parsedResult.toolCallId);
            const timelinePart = persisted
              ? await this.ensureToolTimelinePart(
                  session.id,
                  queued.id,
        ...
        if (controller.signal.aborted) {
          throw new AgentStreamAbort(
            "Agent prompt was cancelled before completion",
          );
        }
        const completed = await this.completeClaimedPrompt(
          session.id,
          queued.id,
        );
        if (!completed) {
        ...
        const aborted =
          controller.signal.aborted || error instanceof AgentStreamAbort;
        await this.failRunningToolCalls(
          session.id,
          queued.id,
          aborted ? "Agent prompt was cancelled" : errorMessage(error),
        );
  ...
  private completeClaimedPrompt(
    ...
    return this.prisma
      .$transaction(async (tx) => {
        ...
        if (sessionCompletion.count !== 1)
          throw new PromptCompletionRaceError();
        return true;
      })
      .catch((error) => {
  ...
  private async snapshot(
    ...
    const [
      ...
      this.prisma.projectAiAgentSession.findUniqueOrThrow({
        where: { id: sessionId },
      }),
      this.prisma.aiAgentMessage.findMany({
        where: { sessionId },
    ...
    const messageIds = [
      ...new Set(
        parts.flatMap((part) => (part.messageId ? [part.messageId] : [])),
      ),
    ];
    const toolCallIds = [
      ...new Set(
        parts.flatMap((part) => (part.toolCallId ? [part.toolCallId] : [])),
      ),
    ];
    const [referencedMessages, referencedToolCalls] = await Promise.all([
      ...
      toolCallIds.length
        ? this.prisma.aiAgentToolCall.findMany({
            where: { sessionId, id: { in: toolCallIds } },
          })
        : [],
    ]);
    const mappedMessages = [...recentMessages].reverse().map(toMessage);
    const mappedToolCalls = [...recentTools]
      .reverse()
      .map((toolCall) => toToolCall(toolCall));
    const hydrationMessages = dedupeById([
      ...recentMessages,
      ...referencedMessages,
    ]).map(toMessage);
    const hydrationToolCalls = dedupeById([
      ...recentTools,
      ...referencedToolCalls,
    ]).map((toolCall) => toToolCall(toolCall));
    ...
    return {
      ...
      timelineInfo: {
        ...
        legacyCursor: compatibilityHasMore
          ? (legacyCursorForUncoveredCompatibility(
              parts,
              recentMessages,
              recentTools,
              projection.timelineInfo.earliestSequence ?? 0,
            ) ??
            legacyCursorForCompatibilityBoundary(
              recentMessages,
              recentTools,
              projection.timelineInfo.earliestSequence ?? 0,
            ))
  ...
  private async legacyTimelinePage(
    ...
    const projection = buildSessionTimelineProjection(
      [],
      messageRows
        .filter((row) => selectedMessageIds.has(row.id))
        .reverse()
        .map(toMessage),
      toolRows
        .filter((row) => selectedToolIds.has(row.id))
        .reverse()
        .map((toolCall) => toToolCall(toolCall)),
    );
    ...
    const nextLegacyCursor =
      hasMore && oldestSource
        ? serializeLegacyTimelineCursor(
            oldestSource.createdAt,
            oldestSource.id,
            parts[0]?.sequence ?? anchor,
          )
        : null;
    return {
      parts,
  ...
  private async broadcast(
    ...
    if (!sessionId) return;
    for (const client of getRuntime(sessionId).clients)
      queueRuntimeEvent(client, event);
  }
  ...
  private async buildPrompts(
    ...
    systemPrompt: string;
    userPrompt: string;
    contextPack: AssembledContextPack;
    skillAllowedTools: string[] | null;
  }> {
    ...
    const transcript = serializeUntrustedData(
      "conversation-history",
      buildRecentTranscript(
        messages.reverse(),
        [activeUserMessageId, activeAssistantMessageId],
        (content) => {
    ...
    for (const skill of activeSkills) {
      if (
        !skill.manifest.runtimeRoles.includes(
          activeAgent?.runtimeRole ?? "orchestrator",
        )
      ) {
        throw new Error(
          `Skill ${skill.name}@${skill.manifest.version} does not authorize runtime role ${activeAgent?.runtimeRole ?? "orchestrator"}`,
        );
      }
    }
    const contextPack = await new ContextAssembler(this.prisma).assemble({
      ...
      sectionKinds: [
        ...new Set(
          activeSkills.flatMap((skill) => skill.manifest.context.sections),
        ),
      ],
    });
    ...
    const finalSystemPrompt = renderInferenceLayers({
      ...
      activeSkills: activeSkills.map((skill) => ({
        ...
        references: loadAiSkillReferences(skill),
      })),
      contextPack,
      ...
      userAuthority: JSON.stringify(
        {
          explicitOwnerInstructionDocs: instructionDocs.map((doc) => ({
            title: doc.title,
    ...
    const userPrompt = renderUserContext({
      transcript,
      prompt:
        attachmentContext || uploadAttachments.length
    ...
    return {
      ...
      userPrompt,
      contextPack,
      skillAllowedTools: activeSkills.length
        ? [
            ...new Set(
              activeSkills.flatMap((skill) => skill.manifest.allowedTools),
            ),
          ]
        : null,
    };
      ...
      if (ref.type === "doc") {
        ...
        const [folders, docs, assets] = await Promise.all([
          this.prisma.projectFolder.findMany({
            where: { projectId, parentFolderId: folder.id },
        ...
        const act = await this.prisma.act.findFirst({
          where: { id: ref.id, projectId },
          include: {
            chapters: {
  ...
  private async handleTask(
    ...
    const callPart = await this.ensureToolTimelinePart(
      parentSessionId,
      parentPromptId,
      "tool-call",
      parentToolCall.id,
    ...
    await this.broadcast(projectId, {
      ...
      data: mergeEventPart(
        toToolCall(parentToolCall),
        await this.hydrateSinglePart(callPart),
      ),
    });
    const { agent, contract, session, payload } = await (async () => {
      if (activeAgent && parentTaskContract)
        throw new HttpError(
          403,
          "Delegated workers cannot invoke other subagents",
        );
      const selectedAgent = findAgent(agents, input.subagent_type);
      if (
        !selectedAgent ||
        (selectedAgent.mode !== "subagent" && selectedAgent.mode !== "all")
      ) {
        throw new HttpError(
          400,
          `Unknown subagent type: ${input.subagent_type}`,
        );
      }
      const selectedContract = input.contract ?? normalizeTaskContract(input);
      ...
      const selectedPayload = normalizePromptPayload({
        ...
        model:
          selectedContract.modelPolicy.preferred ?? selectedAgent.model ?? null,
        attachments: [],
      ...
      await this.finalizeHandlerToolCall(
        parentToolCall,
        projectId,
        "ERROR",
        null,
    ...
    const existingChildMode = fromPrismaApprovalMode(
      session.approvalMode ?? "MANUAL",
    );
    if (
      input.task_id &&
      (payload.buildRunId || existingChildMode !== approvalMode)
    ) {
      await this.prisma.projectAiAgentSession.update({
        where: { id: session.id },
        data: {
    ...
    const startedPart = await this.createTaskPart(
      parentSessionId,
      parentPromptId,
      {
        sessionId: session.id,
    ...
    await this.broadcast(projectId, {
      ...
      data: {
        taskId: session.id,
        part: await this.hydrateSinglePart(startedPart),
      },
    });
    try {
      if (parentAborted)
        throw new AgentStreamAbort("Parent agent prompt was cancelled");
      const last = await this.prisma.aiAgentPrompt.findFirst({
        where: { sessionId: session.id },
        orderBy: { order: "desc" },
        select: { order: true },
      ...
      if (parentAborted || abortSignal?.aborted)
        throw new AgentStreamAbort("Parent agent prompt was cancelled");
      const [result, completedSession, completedPrompt] = await Promise.all([
        ...
        this.prisma.aiAgentPrompt.findUniqueOrThrow({
          where: { id: childPrompt.id },
        }),
      ]);
      if (
        completedSession.status === "CANCELLED" ||
        completedPrompt.status === "CANCELLED"
      ) {
        const message =
          "Subagent was cancelled before completing its assigned task";
        const cancelledPart = await this.createTaskPart(
          parentSessionId,
          parentPromptId,
          {
            sessionId: session.id,
        ...
        await this.broadcast(projectId, {
          ...
          data: {
            taskId: session.id,
            cancelled: true,
            part: await this.hydrateSinglePart(cancelledPart),
          },
        });
        await this.finalizeHandlerToolCall(
          parentToolCall,
      ...
      if (
        completedSession.status !== "IDLE" ||
        completedPrompt.status !== "COMPLETED"
      ) {
        throw new Error(
          completedSession.status === "ERROR" ||
          completedPrompt.status === "ERROR"
            ? (sessionErrorFromSession(completedSession.lastError) ??
              "Subagent failed")
      ...
      const finishedPart = await this.createTaskPart(
        parentSessionId,
        parentPromptId,
        {
          sessionId: session.id,
      ...
      await this.broadcast(projectId, {
        ...
        data: {
          taskId: session.id,
          part: await this.hydrateSinglePart(finishedPart),
        },
      });
      await this.finalizeHandlerToolCall(
        parentToolCall,
      ...
      const failedPart = await this.createTaskPart(
        parentSessionId,
        parentPromptId,
        {
          sessionId: session.id,
      ...
      await this.broadcast(projectId, {
        ...
        data: {
          taskId: session.id,
          error: message,
          part: await this.hydrateSinglePart(failedPart),
        },
      });
      await this.finalizeHandlerToolCall(
        parentToolCall,
  ...
  private async handleQuestion(
    ...
    const timelinePart = await this.ensureToolTimelinePart(
      sessionId,
      promptId,
      "tool-call",
      toolCall.id,
    ...
    return new Promise((resolve, reject) => {
      ...
      const resolveWithCleanup = (value: unknown) => {
        cleanup();
        resolve(value);
      };
      ...
      const onAbort = () => {
        ...
        pendingQuestions.delete(toolCall.id);
        void this.finalizeHandlerToolCall(
          toolCall,
          projectId,
          "ERROR",
          null,
      ...
      pendingQuestions.set(toolCall.id, {
  ...
  private async applyToolCallApproval(
    ...
    if (claimed.count !== 1) {
      const current = await this.prisma.aiAgentToolCall.findUniqueOrThrow({
        where: { id: toolCall.id },
      });
      if (
        approved &&
        ...
        await this.recoverApprovedToolCall(
          userId,
          projectId,
          sessionId,
          current,
      ...
      throw new HttpError(
        409,
        `Tool call was already ${current.status.toLowerCase().replaceAll("_", " ")}`,
      );
    }

    if (!approved) {
      const rejectedCall = await this.prisma.aiAgentToolCall.findUniqueOrThrow({
        where: { id: toolCall.id },
      });
      ...
      await this.broadcast(projectId, {
        ...
        session: await this.snapshot(sessionId, projectId),
        data: mergeEventPart(
          toToolCall(rejectedCall),
          await this.hydrateSinglePart(resultPart),
        ),
      });
      if (!pendingRuntime) {
        await this.failOrphanedInteractiveRun(
    ...
    if (!pendingRuntime) {
      const approvedCall = await this.prisma.aiAgentToolCall.findUniqueOrThrow({
        where: { id: toolCall.id },
      });
    ...
    try {
      ...
      const output = pending
        ...
        : await executeAgentMutationTool(
            this.prisma,
            { projectId, userId },
            toolCall.toolName,
            inputRecord(toolCall.input as Prisma.JsonValue),
          );
      ...
      await this.broadcast(projectId, {
        ...
        session: await this.snapshot(sessionId, projectId),
        data: mergeEventPart(
          toToolCall(failedCall),
          await this.hydrateSinglePart(resultPart),
        ),
      });
      throw new HttpError(
        400,
        `${toolCall.toolName} approval failed: ${message}`,
      );
    }
  }
  ...
  private async createProviderToolCall(
    ...
    if (existing) {
      if (existing.promptId !== promptId || existing.toolName !== toolName) {
        throw new HttpError(
          409,
          `Provider tool call ${toolCallId} is already bound to another invocation`,
        );
      }
    ...
    try {
      ...
      const raced = await this.prisma.aiAgentToolCall.findFirst({
        where: { sessionId, toolCallId },
      });
      if (!raced || raced.promptId !== promptId || raced.toolName !== toolName)
        throw error;
      return raced;
    }
  }
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      await tx.projectAiAgentSession.update({
        ...
        data: {
          status: "ERROR",
          activePromptId: null,
          lastError: mergeSessionMeta(session?.lastError ?? null, {
            error: message,
          }),
        },
      });
  ...
  private async finalizeHandlerToolCall(
    ...
    const updated = await this.prisma.aiAgentToolCall.update({
      ...
      data: {
        status,
        output:
          status === "EXECUTED" ? (output as Prisma.InputJsonValue) : undefined,
        error,
        decidedAt: new Date(),
      },
    });
  ...
  private async hydrateSinglePart(
    part: StoredSessionPart,
  ): Promise<AiAgentSessionPart | null> {
    const [message, toolCall] = await Promise.all([
      part.messageId
        ? this.prisma.aiAgentMessage.findUnique({
            where: { id: part.messageId },
          })
        : null,
      part.toolCallId
        ? this.prisma.aiAgentToolCall.findUnique({
  ...
  private async recoverOrphanedInteractiveSession(
    ...
    if (
      ...
      return false;
    const message =
      "The previous interactive model turn was interrupted by a backend restart. Its persisted trace is available; send or continue with a new prompt.";
  ...
  private async persistToolCall(
    ...
    const approvalRequired =
      MUTATING_TOOLS.has(parsed.toolName) || parsed.toolName === "askUser";
    return toToolCall(
      await this.createProviderToolCall(
        sessionId,
        promptId,
        parsed.toolCallId,
        parsed.toolName,
  ...
  private async failRunningToolCalls(
    sessionId: string,
    promptId: string,
    message: string,
  ): Promise<void> {
  ...
  private async saveContextUsage(
    sessionId: string,
    usage: UsagePayload,
  ): Promise<void> {
...
function safeWriteRuntimeEvent(
...
function queueRuntimeEvent(
  ...
  if (!client.ready) {
    if (client.buffer.length >= 256) {
      const deltaIndex = client.buffer.findIndex(
        (candidate) => candidate.type === "text-delta",
      );
...
export function toToolCall(
  ...
  options: { fullOutput?: boolean; fullInput?: boolean } = {},
): AiAgentToolCall {
  const projectedInput = projectToolInput(
    toolCall.input,
...
export function projectToolOutput(
  ...
  if (full || bytes <= TOOL_OUTPUT_SNAPSHOT_BYTES)
...
function parseToolPart(
  part: unknown,
): { toolCallId: string | null; toolName: string; input: unknown } | null {
  ...
  if (
    ...
    const approvalId =
      typeof record.approvalId === "string" ? record.approvalId : null;
    const toolCallId =
...
export function parseToolResultPart(
  part: unknown,
): { toolCallId: string | null; output: unknown; error: string | null } | null {
  if (!part || typeof part !== "object") return null;
  const record = part as Record<string, unknown>;
  const toolCallId =
    typeof record.toolCallId === "string" ? record.toolCallId : null;
...
export function hasUncoveredTruncatedCompatibility(
  ...
  const coveredCalls = new Set(
    parts.flatMap((part) =>
      part.kind === "tool-call" && part.toolCallId ? [part.toolCallId] : [],
    ),
  );
  const coveredResults = new Set(
    parts.flatMap((part) =>
      part.kind === "tool-result" && part.toolCallId ? [part.toolCallId] : [],
    ),
  );
  const uncoveredMessage = messages.some(
    (message) => !coveredMessages.has(message.id),
  );
  const uncoveredTool = toolCalls.some(
    (toolCall) =>
      !coveredCalls.has(toolCall.id) ||
      (!["RUNNING", "PENDING_APPROVAL", "APPROVED"].includes(toolCall.status) &&
        !coveredResults.has(toolCall.id)),
  );
  return (
    (messagesTruncated && uncoveredMessage) || (toolsTruncated && uncoveredTool)
  );
}
...
function legacyCursorForUncoveredCompatibility(
  ...
  const sources = [
    ...messages
      .filter((message) => !coveredMessages.has(message.id))
      .map((message) => ({ id: message.id, createdAt: message.createdAt })),
    ...toolCalls
      .filter(
        (toolCall) =>
          !coveredCalls.has(toolCall.id) ||
          (!["RUNNING", "PENDING_APPROVAL", "APPROVED"].includes(
            toolCall.status,
          ) &&
            !coveredResults.has(toolCall.id)),
      )
  ...
  return oldest
    ? serializeLegacyTimelineCursor(oldest.createdAt, oldest.id, beforeSequence)
    : null;
}
...
function serializeLegacyTimelineCursor(
  createdAt: Date,
  id: string,
  beforeSequence: number,
): string {
...
function parseLegacyTimelineCursor(
  value: string | undefined,
): { createdAt: Date; id: string; beforeSequence: number | null } | null {
...
function mergeEventPart(
  value: unknown,
  part: AiAgentSessionPart | null,
...
function validateQuestionAnswers(input: unknown, answers: string[][]): void {
  ...
  if (expected === 0)
    throw new HttpError(
      400,
      "Question payload is malformed and cannot be answered",
    );
  if (
    ...
    .map((question, index) => {
      ...
      const label =
        typeof record.question === "string"
          ? record.question
          : `Question ${index + 1}`;
      const answer = answers[index]?.length
        ? answers[index].join(", ")
        : "Unanswered";
      return `"${label}"="${answer}"`;
    })
...
function parsePromptPayload(value: string): PromptPayload {
  try {
    ...
    if (parsed.type !== "opentales.aiPrompt")
      throw new Error("Not an AI prompt payload");
...
function selectActiveSkills<
  T extends { name: string; manifest: { version: string; kind?: string } },
>(skills: T[], task: TaskContract | null, agent: AiAgentInfo | undefined): T[] {
  const declared = Object.entries(task?.skillVersions ?? {});
  if (declared.length)
    return declared.map(([name, version]) => {
      const skill = skills.find((candidate) => candidate.name === name);
      if (!skill)
        throw new Error(`Pinned skill ${name}@${version} is unavailable`);
      if (skill.manifest.version !== version)
        throw new Error(
          `Pinned skill ${name}@${version} resolved to ${skill.manifest.version}`,
        );
      return skill;
    });
  const inferred = agentSkillName(agent?.name);
  const skill = inferred
    ? skills.find((candidate) => candidate.name === inferred)
    : undefined;
  return skill ? [skill] : [];
}
...
function agentSkillName(agentName: string | undefined): string | null {
  ...
  if (
    agentName.includes("chapter-writer") ||
    agentName.includes("chapter-continuity")
  )
    return "novel-chapters";
  if (agentName.includes("critic")) return "novel-critic";
  const match = agentName.match(
    /^(idea|characters|settings|perspective|voice|obstacles|outline|climax)-runner$/,
  );
...
function sanitizeProjectReference(
  value: unknown,
): AiAgentAttachment["reference"] {
...
function referenceContent(
  content: string,
  ref: NonNullable<AiAgentAttachment["reference"]>,
): string {
...
function usageFromPart(
  ...
  const outputTokens =
    ...
    0;
  const totalTokens =
    numberValue(usage.totalTokens) ?? inputTokens + outputTokens;
  ...
  return {
    ...
    percentage: Math.min(
      100,
      Math.round((totalTokens / maxTokens) * 1000) / 10,
    ),
    model,
  };
}
...
export function classifyAgentStreamTerminal(
  part: unknown,
): { kind: "abort" | "error"; message: string } | null {
  const type = stringValue(objectRecord(part)?.type);
  if (type === "abort")
    return {
      kind: "abort",
      message: streamPartError(part) ?? "Model stream aborted",
    };
  if (type === "error")
    return {
      kind: "error",
      message: streamPartError(part) ?? "Model stream failed",
    };
...
function contextWindowForModel(model: string | null): number {
  ...
  if (id.includes("gpt-5") || id.includes("gpt-4.1") || id.includes("gemini"))
    return 1_000_000;
...
function errorMessage(error: unknown): string {
  ...
  if (!error || typeof error !== "object")
    return error instanceof Error ? error.message : "AI agent failed";
...
function usageFromError(
  ...
  const match = message.match(
    /Prompt tokens limit exceeded:\s*(\d+)\s*>\s*(\d+)/i,
  );
  ...
  const next = {
    ...
    contextUsage:
      update.contextUsage === undefined
        ? contextUsageFromSession(lastError)
        : update.contextUsage,
    error:
      update.error === undefined
        ? sessionErrorFromSession(lastError)
        : update.error,
  };
...
function contextUsageFromSession(
  ...
  return {
    ...
    maxTokens,
    percentage:
      numberValue(usage.percentage) ??
      Math.min(100, Math.round((totalTokens / maxTokens) * 1000) / 10),
