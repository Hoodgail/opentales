...
export function renderInferenceLayers(input: LayeredInferenceInput): string {
  return [
    ...
    renderSkillLayer(
      input.activeSkills ??
        (input.activeSkill ? [{ manifest: input.activeSkill }] : []),
      input.activeAgentInstructions,
    ),
...
function renderWorkflowLayer(
  ...
  if (!task)
    return [
      "# Layer B — Active task",
      "Use project docs to maintain an adaptable plan and progress. Read existing work, choose the next useful action, and write or edit through the available tools. Respect the selected approval mode.",
    ].join("\n");
  return [
          ...
          .map((skill) =>
            [
              ...
              ...(skill.references ?? []).map(
                (reference) =>
                  `<skill_reference name="${escapeAttribute(reference.name)}">\n${reference.content}\n</skill_reference>`,
              ),
              "</active_skill>",
            ]
              .filter(Boolean)
...
function renderContextLayer(pack: AssembledContextPack | null): string {
  return [
    ...
    pack?.text ||
      "No story-state context was selected. Retrieve missing facts through bounded story search tools.",
...
function renderAuthorityLayer(userAuthority?: string): string {
  return [
    ...
    userAuthority?.trim()
      ? serializeAuthorityData(userAuthority)
      : "No additional project-level creative constraints were supplied.",
  ].join("\n");
}
...
function renderOutputLayer(task: TaskContract | null): string {
  ...
  return [
    ...
    "Persist changes through the available writing tools. Return a concise summary of completed work, document IDs, verification, and any unresolved questions.",
