...
function productionPlanningArtifactsFor(
  ...
  return types.flatMap((type) => {
    ...
    if (type === 'plot-thread') return Array.from({ length: 5 }, (_, index) => operation(type, index === 0 ? 'plot-thread:main-thread' : `thread-${index + 1}`, index === 0 ? 'Main Restoration' : `Supporting Thread ${index + 1}`, { threadKey: index === 0 ? 'main-thread' : `thread-${index + 1}`, kind: index === 0 ? 'main' : index === 1 ? 'mystery' : index === 2 ? 'character-arc' : index === 3 ? 'romance' : 'thematic', summary: `Causal thread ${index + 1} crosses the manuscript.`, stakes: 'Memory, identity, and the surviving city.', characterRefs: [characterRefs[index % characterRefs.length]], beatKeys: beatKeys.filter((_, beatIndex) => beatIndex % 5 === index), setupPayoffKeys: index === 0 ? ['memory-price'] : [], resolution: `Thread ${index + 1} resolves in the finale.` }));
    if (type === 'beat') {
    ...
    if (type === 'finale-plan') return [operation(type, 'finale', 'Production Finale', { finaleKey: 'finale', mainThreadKey: 'plot-thread:main-thread', resolvesMainThread: true, climax: 'The cartographer draws the final district knowing who will forget her.', endingCost: 'The beloved survives without recognition.', thematicResolution: 'Preservation without possession remains an act of love.', intentionallyOpenLoopKeys: [] })];
    return [];
  });
}
