...
describe("story editing tool contracts", () => {
  ...
  it("supports deterministic chapter compilation from revision-pinned scenes", () => {
    expect(
      mutationToolSchemas.compileChapterFromScenes.parse({
        chapterId: "chapter-1",
  ...
  it("requires read-before-write concurrency tokens for prose changes", () => {
    expect(() =>
      mutationToolSchemas.updateChapter.parse({
        chapterId: "chapter-1",
        content: "Replacement",
      }),
    ...
    expect(() =>
      mutationToolSchemas.updateScene.parse({
        sceneId: "scene-1",
        expectedRevision: 3,
        content: "Replacement",
      }),
  ...
  it("edits open submissions in place and rejects mutually exclusive body modes", () => {
    expect(
      mutationToolSchemas.updateSubmission.parse({
    ...
    expect(() =>
      mutationToolSchemas.mergeSubmission.parse({
        submissionId: "submission-1",
  ...
  it("applies ordered exact edits and supports explicit empty replacements", () => {
    expect(applyContentPatch("", { mode: "replace", content: "Draft" })).toBe(
      "Draft",
    );
    expect(
      applyContentPatch("Lio entered Vale. Lio left.", {
        mode: "edit",
        edits: [
          { oldString: "Lio", newString: "Neri", replaceAll: true },
  ...
  it("accepts atomic mixed-target patches with scene-specific revision guards", () => {
    expect(
      mutationToolSchemas.applyStoryPatch.parse({
        ...
        operations: [
          {
            ...
            patch: {
              mode: "edit",
              edits: [{ oldString: "Lio", newString: "Neri" }],
            },
          },
          {
            target: "scene",
