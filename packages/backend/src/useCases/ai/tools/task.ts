...
const taskInputSchema = z
  .object({
    description: z
      ...
      .trim()
      .min(1)
      .describe("Short description of the task for UI/session title"),
    contract: taskContractSchema
      .optional()
      .describe(
        "Typed executable assignment. Required for new durable-workflow delegations.",
      ),
    objective: z
      .string()
      .trim()
      ...
      .optional()
      .describe(
        "Legacy-compatible objective; normalized into a typed contract.",
      ),
    prompt: z
      .string()
      .trim()
      .min(1)
      .optional()
      .describe("Deprecated legacy prompt; normalized into a typed contract."),
    subagent_type: z
      .string()
      .trim()
      .min(1)
    ...
    task_id: z
      ...
      .trim()
      .optional()
      .describe("Existing subagent session id to resume"),
  })
  .refine(
    (input) => Boolean(input.contract || input.objective || input.prompt),
    {
...
function taskDescription(subagents: AiAgentInfo[]): string {
  ...
  return [
    ...
    "Pass task_id to resume a previous subagent session returned by this tool. Resolve opaque IDs with list tools.",
    "",
    "Available subagents:",
    available,
  ].join("\n");
