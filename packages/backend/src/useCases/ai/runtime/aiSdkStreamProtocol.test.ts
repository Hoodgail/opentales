...
describe("AI SDK stream protocol integration", () => {
  it("propagates exact provider IDs for parallel same-name calls and exposes one call/result pair each", async () => {
    ...
    const order: string[] = [];
    const handler = vi.fn(
      async (input: { description: string }, toolCallId: string) => {
    ...
    expect(new Set(invocations.map((invocation) => invocation.id)).size).toBe(
      2,
    );
    expect(
      parts
        ...
        .sort(),
    ).toEqual(["provider-call-a", "provider-call-b"]);
    expect(
      parts
    ...
    expect(order.indexOf("execute:provider-call-a")).toBeLessThan(
      order.indexOf("stream:tool-call:provider-call-a"),
    );
    expect(order.indexOf("execute:provider-call-b")).toBeLessThan(
      order.indexOf("stream:tool-call:provider-call-b"),
    );
  });
  ...
  it("keeps parallel same-name mutation approvals distinct by provider invocation ID", async () => {
    ...
    const model = modelWithChunks([
      ...
      {
        type: "tool-call",
        toolCallId: "chapter-call-a",
        toolName: "createChapter",
        input: JSON.stringify({ title: "Chapter A" }),
      },
      {
        type: "tool-call",
        toolCallId: "chapter-call-b",
        toolName: "createChapter",
        input: JSON.stringify({ title: "Chapter B" }),
      },
      finish("tool-calls"),
    ]);
    const result = streamText({
    ...
    expect(new Set(approvals.map((approval) => approval.toolCallId)).size).toBe(
      2,
    );
  });
  ...
  it("outer-persists and exposes one ERROR call/result when an invalid task never reaches its handler", async () => {
    ...
    const handlerOwned =
      isHandlerManagedTool("task") && invalidCall.invalid !== true;
    ...
    if (
      shouldBroadcastOuterToolEvent(
        "task",
        "tool-error",
        !outerOwned.has(invalidResult.toolCallId),
      )
    ) {
  ...
  it("classifies the generic error part emitted by a real streamText pipeline with its exact message", async () => {
    ...
    expect(terminals[0]).toEqual({
      kind: "error",
      message: "provider database exploded",
    });
    expect(terminals.at(-1)).toEqual({
      kind: "error",
      message: "Model stream finished with an error",
    });
  });

  it("classifies abort and error finish events as terminal rather than successful completion", () => {
    expect(
      classifyAgentStreamTerminal({ type: "abort", reason: "user cancelled" }),
    ).toEqual({
      kind: "abort",
      message: "user cancelled",
    });
    expect(
      classifyAgentStreamTerminal({ type: "finish", finishReason: "error" }),
    ).toEqual({
      kind: "error",
      message: "Model stream finished with an error",
    });
  ...
  it("preserves exact SDK tool errors and denial terminal messages", () => {
    ...
    expect(
      parseToolResultPart({ type: "tool-output-denied", toolCallId: "c" }),
        ...
        async ({ where }: { where: Record<string, unknown> }) =>
          rows.find(
            (row) =>
              ...
              row.toolCallId === where.toolCallId,
          ) ?? null,
      ),
