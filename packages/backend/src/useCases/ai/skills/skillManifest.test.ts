...
describe("skill manifests", () => {
  ...
  it("declares bounded procedure, role, tools, and context", () => {
    const manifest = parseSkillManifest({
      ...
      name: "writing-plan",
      version: "1.0.0",
      description: "Plan writing in project docs.",
      kind: "workflow",
      inputs: ["story-brief"],
  ...
  it("does not ship the retired workflow entrypoint", () => {
    expect(
      loadBuiltInAiSkills().some((skill) => skill.name === "novel-build"),
    ).toBe(false);
  });

  it("publishes document editing capability in the continuity skill contract", () => {
    const continuity = loadBuiltInAiSkills().find(
      (candidate) => candidate.name === "novel-continuity",
    );
    expect(continuity?.manifest.version).toBe("1.2.0");
    expect(continuity?.manifest.allowedTools).toContain("updateProjectDoc");
  });

  it("gives every shipped skill a matching versioned machine contract", async () => {
    const here = dirname(fileURLToPath(import.meta.url));
    const directories = (await readdir(here, { withFileTypes: true })).filter(
    ...
    const manifests = await Promise.all(
      directories.map(async (entry) =>
        parseSkillManifest(
          JSON.parse(
            await readFile(resolve(here, entry.name, "skill.json"), "utf8"),
          ),
    ...
    for (const manifest of manifests) {
      expect(manifest.name).toBe(
        directories.find((entry) => entry.name === manifest.name)?.name,
      );
  ...
  it("rejects mismatched project-owned skill provenance", () => {
    expect(() =>
      projectSkillManifest(
        "safe-name",
        "A sufficiently useful description",
        [
          "---",
          "name: different-name",
          "version: 2.0.0",
  ...
  it("validates explicit project-owned runtime permissions and context instead of silently widening them", () => {
    const manifest = projectSkillManifest(
      "project-drafter",
      "A scoped project drafting procedure",
      [
        "---",
