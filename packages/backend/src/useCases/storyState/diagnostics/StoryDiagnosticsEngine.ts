...
import type {
  StoryDiagnosticsEngineOptions,
  StoryDiagnosticsInput,
} from "./types.js";

/**
 * Deterministic diagnostics over one transactionally consistent story snapshot
...
export function runStoryDiagnostics(
...
function assertInput(input: StoryDiagnosticsInput): void {
  if (!input || typeof input !== "object")
    throw new TypeError("Story diagnostics input is required");
  if (!input.projectId?.trim())
    throw new TypeError("Story diagnostics projectId is required");
  if (!input.buildRunId?.trim())
    throw new TypeError("Story diagnostics buildRunId is required");
  for (const key of [
    "chapters",
    "characters",
    "locations",
    ...
    if (!Array.isArray(input[key]))
      throw new TypeError(`Story diagnostics ${key} must be an array`);
  }
  for (const chapter of input.chapters) {
    if (!chapter.id?.trim())
      throw new TypeError("Every diagnostic chapter requires an id");
    if (!Number.isFinite(chapter.number))
      throw new TypeError(`Chapter ${chapter.id} requires a finite number`);
    if (!Array.isArray(chapter.scenes))
      throw new TypeError(`Chapter ${chapter.id} scenes must be an array`);
    for (const scene of chapter.scenes) {
      if (!scene.id?.trim())
        throw new TypeError(
          `Every scene in chapter ${chapter.id} requires an id`,
        );
      if (!Number.isFinite(scene.order))
        throw new TypeError(`Scene ${scene.id} requires a finite order`);
      if (typeof scene.content !== "string")
        throw new TypeError(`Scene ${scene.id} content must be a string`);
    }
  }
}
