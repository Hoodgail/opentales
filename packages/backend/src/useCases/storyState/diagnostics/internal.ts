...
export function buildDiagnosticContext(
  ...
  const enabledCategories = new Set(
    input.metadata?.enabledCategories ??
      input.projectRules?.enabledCategories ??
      [],
  );
  ...
  return {
    ...
    activeArtifacts: input.artifacts.filter(
      (artifact) =>
        artifact.status !== "invalidated" && artifact.status !== "superseded",
    ),
    ...
    add(draft) {
      ...
      if (enabledCategories.size > 0 && !enabledCategories.has(draft.category))
        return;
...
function normalizeDraft(draft: DiagnosticDraft): DiagnosticDraft {
  return {
    ...
    evidence: uniqueBy(draft.evidence.filter(hasSourceSpan), (span) =>
...
function compareSourceSpans(
  left: StorySourceSpan,
  right: StorySourceSpan,
): number {
...
export function chapterEvidence(
  ...
  return {
    ...
    ...(chapter.sourceArtifactId
      ? { artifactId: chapter.sourceArtifactId }
      : {}),
    ...(details.start !== undefined ? { start: details.start } : {}),
    ...(details.end !== undefined ? { end: details.end } : {}),
    ...(details.quote ? { quote: excerpt(details.quote) } : {}),
...
export function sourceEvidence(value: {
  ...
  if (value.sourceSpan && hasSourceSpan(value.sourceSpan))
...
export function findEvidence(
  ...
  const haystack = caseSensitive
    ? unit.text
    : unit.text.toLocaleLowerCase("en-US");
...
export function parseSceneTimestamp(
  scene: DiagnosticSceneSnapshot,
): number | null {
