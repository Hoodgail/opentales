...
function runArtifactIntegrityRules(context: DiagnosticContext): void {
  for (const artifact of context.activeArtifacts) {
    try {
      ...
      context.add({
        ...
        suggestedResolution:
          "Repair the artifact content against its declared schema before using it downstream.",
      });
    }
  }
  ...
  for (const artifacts of duplicateArtifactKeys.values()) {
    ...
    context.add({
      ...
      suggestedResolution:
        "Supersede or invalidate all but the intended current artifact.",
    });
  }
}
...
function runReferenceIntegrityRules(context: DiagnosticContext): void {
  const chapterKeys = new Set(
    context.activeArtifacts
      .filter(
        (artifact) =>
          artifact.type === "chapter-brief" && isJsonObject(artifact.content),
      )
      .map((artifact) =>
        String((artifact.content as JsonObject).chapterKey ?? artifact.key),
      ),
  );
  const scenePlans = context.activeArtifacts.filter(
    (artifact) =>
      artifact.type === "scene-plan" && isJsonObject(artifact.content),
  );
  const sceneKeys = new Set(
    scenePlans.map((artifact) =>
      String((artifact.content as JsonObject).sceneKey ?? artifact.key),
    ),
  );
  ...
  for (const artifact of scenePlans) {
    ...
    if (chapterKey && !chapterKeys.has(chapterKey)) {
      context.add({
        ...
        suggestedResolution:
          "Create the chapter brief or relink the scene plan to an active chapter.",
      });
    }
    for (const dependency of stringArray(content.dependencies)) {
      ...
      context.add({
        ...
        suggestedResolution:
          "Restore the dependency or remove the stale dependency key.",
      });
    }
  }
  ...
  const checkSceneLink = (
    ...
    context.add({
      ...
      evidence: [],
      relatedRefs: [
        reference(ownerType, ownerId, ownerKey),
        reference("scene", sceneId),
      ],
      suggestedResolution:
        "Relink the record to a scene on this build branch or invalidate the stale record.",
    });
  };

  for (const fact of context.input.canonFacts.filter(
    (value) => value.isCurrent && value.invalidatedAt === null,
  )) {
    checkSceneLink(
      "canon-fact",
      fact.id,
  ...
  for (const state of context.input.entityStates.filter(
    (value) => value.isCurrent && value.invalidatedAt === null,
  )) {
    checkSceneLink(
      "entity-state",
  ...
  for (const setup of context.input.setupPayoffs.filter(
    (value) => value.isCurrent && value.invalidatedAt === null,
  )) {
    checkSceneLink(
      "setup-payoff",
...
function runCanonContinuityRules(context: DiagnosticContext): void {
  ...
  const slots = groupBy(
    canonical,
    (fact) =>
      `${normalizeKey(fact.subjectType)}:${fact.subjectId}:${normalizeKey(fact.predicate)}`,
  );
  ...
  for (const facts of slots.values()) {
    const conflicting = pairs(facts).filter(
      ([left, right]) =>
        ...
        intervalsOverlap(
          factInterval(left, context),
          factInterval(right, context),
        ),
    );
    if (conflicting.length === 0) continue;
    const involved = uniqueBy(conflicting.flat(), (fact) => fact.id);
    const first = involved[0];
    const category = /world|rule|magic|technology/.test(
      normalizeKey(first.subjectType),
    )
      ? "world-rule"
      : "continuity";
    context.add({
      ...
      relatedRefs: involved.map((fact) =>
        reference("canon-fact", fact.id, fact.key),
      ),
      suggestedResolution:
        "Reconcile the values or close the earlier fact’s validity interval.",
    });
  }
}
...
function runEntityStateRules(context: DiagnosticContext): void {
  ...
  const slots = groupBy(
    activeStates,
    (state) =>
      `${normalizeKey(state.entityType)}:${state.entityId}:${normalizeKey(state.stateKey)}`,
  );
  ...
  for (const states of slots.values()) {
    const conflicting = pairs(states).filter(
      ([left, right]) =>
        ...
        intervalsOverlap(
          stateInterval(left, context),
          stateInterval(right, context),
        ),
    );
    if (conflicting.length === 0) continue;
    const involved = uniqueBy(conflicting.flat(), (state) => state.id);
    const first = involved[0];
    const locationState = /location|place|whereabouts/.test(
      normalizeKey(first.stateKey),
    );
    context.add({
      ...
      relatedRefs: involved.map((state) =>
        reference("entity-state", state.id, state.key),
      ),
      suggestedResolution:
        "Close the prior state interval or reconcile the state values.",
    });
  }
  ...
  for (const scene of context.scenes) {
    ...
    for (const characterId of uniqueBy(
      ...
      if (
        ...
        (current.validFromSceneId === scene.id ||
          current.sourceSpan?.sceneId === scene.id)
      ) {
      ...
      context.add({
        ...
        suggestedResolution:
          "Remove the appearance, move it before the death, or explicitly establish a permitted return.",
      });
    }
  }
}
...
function runAppearanceContinuityRules(context: DiagnosticContext): void {
  ...
  for (const character of context.input.characters) {
    const names = uniqueBy(
      [character.name, ...(character.aliases ?? [])].filter(Boolean),
      (value) => value,
    );
    for (const attribute of attributes) {
      ...
      context.add({
        ...
        relatedRefs: [
          reference("character", character.id, character.key, character.name),
        ],
...
function runObjectTransferContinuityRules(
  ...
  const ownershipStates = activeStates.filter(
    (state) =>
      !/^(character|person)$/.test(normalizeKey(state.entityType)) &&
      /owner|ownership|holder|possessor|possession/.test(
        normalizeKey(state.stateKey),
      ),
  );
  ...
  const transfers = context.scenes.flatMap((scene) =>
    sceneObjectTransfers(scene).map((transfer) => ({
      ...transfer,
      scene,
      order: sceneRank(context, scene.id),
    })),
  );
  ...
  for (const states of byObject.values()) {
    ...
    for (let index = 1; index < sorted.length; index += 1) {
      ...
      context.add({
        ...
        suggestedResolution:
          "Record the intervening handoff/loss/theft/destruction or correct the ownership states.",
      });
    }
  }
}
...
function runChronologyRules(context: DiagnosticContext): void {
    ...
    for (const scene of context.scenes) {
      ...
      if (
        ...
        context.add({
          ...
          suggestedResolution:
            "Correct the dates or mark the scene as an intentional flashback/flash-forward.",
        });
      }
      previous = scene;
  ...
  const events = context.input.timelineEvents.filter(
    (event) => event.isCurrent && event.invalidatedAt === null,
  );
    ...
    for (const dependencyId of event.dependencyIds) {
      ...
      if (!dependency) {
        context.add({
          ...
          suggestedResolution:
            "Restore the prerequisite event or remove the stale dependency.",
        });
        continue;
      }
      const dependencyOrder =
        dependency.sortOrder ?? parseChronology(dependency.chronology);
      if (
        eventOrder === null ||
        dependencyOrder === null ||
        eventOrder > dependencyOrder
      )
        continue;
      context.add({
        ...
        suggestedResolution:
          "Move the prerequisite earlier or repair the dependency direction.",
      });
    }
  }
  ...
  for (const [characterId, values] of appearances) {
    const sorted = values.sort(
      (left, right) =>
        ...
        sceneRank(context, left.scene.id) - sceneRank(context, right.scene.id),
    );
    for (let index = 1; index < sorted.length; index += 1) {
      const previous = sorted[index - 1];
      const current = sorted[index];
      if (previous.locationId === current.locationId) continue;
      const minimum = minimumTravel.get(
      ...
      context.add({
        ...
        suggestedResolution:
          "Increase the elapsed story time, change a location, or establish faster transportation in canon.",
      });
    }
  }
}
...
function runLocationRules(context: DiagnosticContext): void {
  ...
  for (const [key, scenes] of simultaneous) {
    ...
    context.add({
      ...
      suggestedResolution:
        "Correct the time/location metadata or separate the appearances chronologically.",
    });
  }
  ...
  for (const scene of context.scenes) {
    ...
    for (const characterId of characters) {
      ...
      if (
        !current ||
        !recordedLocation ||
        recordedLocation === scene.locationId
      )
        continue;
      ...
      context.add({
        ...
        suggestedResolution:
          "Record the intervening movement or correct the scene/state location.",
      });
    }
  }

  for (const scene of context.scenes) {
    if (!scene.locationId || context.locationById.has(scene.locationId))
      continue;
    context.add({
      ...
      relatedRefs: [
        sceneReference(scene),
        reference("location", scene.locationId),
      ],
      suggestedResolution:
        "Create the location or choose an existing location from the Story Bible.",
    });
  }
}
...
function runKnowledgeRules(context: DiagnosticContext): void {
  ...
  const events = new Map<
    string,
    Array<{ order: number; operation: "gain" | "lose" }>
  >();
  const claims: Array<{
    scene: DiagnosticSceneSnapshot;
    claim: DiagnosticKnowledgeClaim;
  }> = [];
  ...
  for (const scene of context.scenes) {
    ...
    for (const delta of sceneKnowledgeDeltas(scene)) {
      ...
      events.set(key, [
        ...(events.get(key) ?? []),
        { order, operation: delta.operation },
      ]);
    }
    for (const claim of sceneKnowledgeClaims(scene))
      claims.push({ scene, claim });
  }
  ...
  for (const { scene, claim } of claims) {
    ...
    const futureScene = Number.isFinite(futureOrder)
      ? context.scenes[futureOrder]
      : undefined;
    context.add({
      code: Number.isFinite(futureOrder)
      ...
      evidence: [
        evidence,
        ...(futureScene ? [sceneEvidence(futureScene)] : []),
      ],
      relatedRefs: [
        reference(
          "character",
          claim.characterId,
          undefined,
          characterName(context, claim.characterId),
        ),
        sceneReference(scene),
...
function runWorldRuleRules(context: DiagnosticContext): void {
  const worldArtifacts = context.activeArtifacts.filter(
    (artifact) =>
      artifact.type === "world-bible" && isJsonObject(artifact.content),
  );
  const rules = new Map<
    string,
    Array<{ statement: string; artifact: StoryArtifact }>
  >();
  for (const artifact of worldArtifacts) {
    ...
    for (const entry of entries) {
      ...
      rules.set(normalizeKey(key), [
        ...(rules.get(normalizeKey(key)) ?? []),
        { statement, artifact },
      ]);
    }
  }
  ...
  for (const [key, definitions] of rules) {
    const statements = uniqueBy(
      definitions.map((value) => normalizeText(value.statement)),
      (value) => value,
    );
    ...
    context.add({
      ...
      relatedRefs: definitions.map((definition) =>
        reference(
          ...
          definition.artifact.key,
          definition.artifact.title,
        ),
      ),
      suggestedResolution:
        "Keep one canonical definition or version the rule with explicit validity.",
    });
  }
  ...
  for (const scene of context.scenes) {
    for (const ruleKey of worldRuleReferences(scene.worldRuleRefs)) {
      ...
      context.add({
        ...
        relatedRefs: [
          sceneReference(scene),
          reference("world-rule", ruleKey, ruleKey),
        ],
        suggestedResolution:
          "Define the rule in the active World Bible or correct the reference.",
      });
    }
    for (const violation of scene.worldRuleViolations ?? []) {
      ...
      context.add({
        ...
        evidence: [
          ...
          ...(rules.get(normalizeKey(violation.ruleKey)) ?? []).map(
            (definition) => artifactEvidence(definition.artifact.id),
          ),
        ],
        relatedRefs: [
          sceneReference(scene),
          reference("world-rule", violation.ruleKey, violation.ruleKey),
        ],
        suggestedResolution:
          "Revise the event or intentionally amend the canonical world rule.",
      });
    }
  }
}
...
function runCharacterRules(context: DiagnosticContext): void {
  ...
  for (const artifact of context.activeArtifacts) {
    if (artifact.type !== "character-bible" || !isJsonObject(artifact.content))
      continue;
    for (const id of [
      artifact.id,
      artifact.key,
      jsonString(artifact.content.characterKey),
    ]) {
  ...
  for (const scene of context.scenes) {
    const characterIds = uniqueBy(
      [
        ...
        ...(scene.normalizedKnowledgeDeltas ?? []).map(
          (delta) => delta.characterId,
        ),
        ...(scene.characterSignals ?? []).map((signal) => signal.characterId),
        ...(scene.dialogueTurns ?? []).flatMap((turn) =>
    ...
    for (const characterId of characterIds) {
      ...
      context.add({
        ...
        relatedRefs: [
          sceneReference(scene),
          reference("character", characterId),
        ],
        suggestedResolution:
          "Create the character in the Story Bible or correct the reference.",
      });
    }
      ...
      context.add({
        ...
        relatedRefs: [
          reference(
            ...
            signal.characterId,
            undefined,
            characterName(context, signal.characterId),
          ),
          sceneReference(scene),
        ],
  ...
  const bibleArtifacts = context.activeArtifacts.filter(
    (artifact) =>
      artifact.type === "character-bible" && isJsonObject(artifact.content),
  );
  const byKey = groupBy(bibleArtifacts, (artifact) =>
    normalizeKey(
      jsonString((artifact.content as JsonObject).characterKey) ?? artifact.key,
    ),
  );
  for (const artifacts of byKey.values()) {
    ...
    context.add({
      ...
      suggestedResolution:
        "Merge the character records and supersede the obsolete artifacts.",
    });
  }
}
...
function runPovRules(context: DiagnosticContext): void {
  const rules =
    context.input.projectRules?.pov ?? narrativeContractPovRules(context);
  const allowed = new Set(rules.allowedCharacterIds ?? []);
  for (const scene of context.scenes) {
    if (
      rules.requiredCharacterId &&
      scene.povCharacterId !== rules.requiredCharacterId
    ) {
      context.add({
        ...
        suggestedResolution:
          "Change the scene POV or explicitly revise the narrative contract.",
      });
    } else if (
      allowed.size > 0 &&
      scene.povCharacterId &&
      !allowed.has(scene.povCharacterId)
    ) {
      context.add({
        code: "forbidden-pov-character",
        category: "pov",
        ...
        relatedRefs: [
          sceneReference(scene),
          reference("character", scene.povCharacterId),
        ],
        suggestedResolution:
          "Use an allowed POV or update the project’s narrative contract.",
      });
    }

    const interiority = uniqueBy(
      scene.interiorityCharacterIds ?? [],
      (value) => value,
    );
    if (
      ...
      context.add({
        ...
        relatedRefs: [
          sceneReference(scene),
          ...interiority.map((id) => reference("character", id)),
        ],
        suggestedResolution:
          "Keep interior access with the POV character or mark a deliberate POV break.",
      });
    }
  }
  ...
  if (rules.singlePovPerChapter) {
    for (const chapter of context.chapters) {
      ...
      context.add({
        ...
        evidence: chapter.scenes
          .filter((scene) => scene.povCharacterId)
          .map((scene) => sceneEvidence(scene)),
        relatedRefs: [
          chapterReference(chapter),
          ...povIds.map((id) => reference("character", id)),
        ],
        suggestedResolution:
          "Split the chapter at the POV change or keep one viewpoint.",
      });
    }
  }
}
...
function runSetupPayoffRules(context: DiagnosticContext): void {
  const setups = context.input.setupPayoffs.filter(
    (setup) =>
      setup.isCurrent &&
      setup.invalidatedAt === null &&
      setup.status !== "invalidated" &&
      setup.status !== "abandoned",
  );
  for (const setup of setups) {
    const setupEvidence = linkedEvidence(
      setup.setupSceneId,
      setup.setupArtifactId,
      context,
    );
    const payoffEvidence = linkedEvidence(
      setup.payoffSceneId,
      setup.payoffArtifactId,
      context,
    );
    const reinforcementEvidence = setup.reinforcementSceneIds.flatMap(
      (sceneId) =>
        context.sceneById.has(sceneId)
          ? [sceneEvidence(context.sceneById.get(sceneId)!)]
          : [],
    );
    if (setup.status === "paid-off" && setupEvidence.length === 0) {
      context.add({
        ...
        relatedRefs: [
          reference("setup-payoff", setup.id, setup.key, setup.title),
        ],
        suggestedResolution:
          "Link an earlier setup scene/artifact or revise the payoff status.",
      });
    }
    if (
      ["setup", "reinforced"].includes(setup.status) &&
      payoffEvidence.length === 0
    ) {
      context.add({
        code: "unpaid-setup",
        category: "setup-payoff",
        ...
        suggestedResolution:
          "Plan and link a payoff, or intentionally abandon the setup.",
      });
    }
    ...
    if (
      ...
      context.add({
        ...
        suggestedResolution:
          "Move the setup earlier, move the payoff later, or correct the links.",
      });
    }
    for (const sceneId of setup.reinforcementSceneIds) {
      ...
      if (
        ...
        context.add({
          ...
          suggestedResolution:
            "Relink or reorder the reinforcement so it strengthens the established promise.",
        });
      }
    }
  }
  ...
  for (const loop of context.input.openLoops.filter(
    ...
    const resolvedEvidence = linkedEvidence(
      loop.resolvedSceneId,
      loop.resolvedArtifactId,
      context,
    );
    if (loop.status === "resolved" && resolvedEvidence.length === 0) {
      context.add({
        ...
        suggestedResolution:
          "Link the resolving scene/artifact or return the loop to an open status.",
      });
    } else if (
      isManuscriptComplete(context.input) &&
      ["open", "reinforced"].includes(loop.status)
    ) {
      context.add({
        code: "unresolved-open-loop",
        category: "setup-payoff",
        severity:
          loop.kind === "mystery" || loop.kind === "promise"
            ? "error"
            : "warning",
        ...
        relatedRefs: [reference("open-loop", loop.id, loop.key, loop.title)],
        suggestedResolution:
          "Resolve the loop on-page or mark it intentionally abandoned.",
      });
    }
  }
}
...
function runPlotRules(context: DiagnosticContext): void {
  ...
  for (const item of plannedDependencies) {
    for (const dependencyId of item.dependencyIds) {
      const dependency = plannedDependencies.find(
        (candidate) =>
          candidate.id === dependencyId || candidate.key === dependencyId,
      );
      if (!dependency) {
        context.add({
          ...
          suggestedResolution:
            "Restore the predecessor, relink the dependency, or revise the causal chain.",
        });
        ...
        context.add({
          ...
          suggestedResolution:
            "Reorder the scenes or correct the dependency direction.",
        });
      }
    }
  }
  const cycle = hasCycle(
    plannedDependencies.map((item) => ({
      ...
      dependencyIds: item.dependencyIds
        .map(
          (dependency) =>
            plannedDependencies.find(
              (candidate) =>
                candidate.id === dependency || candidate.key === dependency,
            )?.id,
        )
  ...
  if (cycle) {
    ...
    context.add({
      ...
      suggestedResolution:
        "Break the circular dependency so every scene has an executable causal order.",
    });
  }
  ...
  if (requireDependencies) {
    for (const item of plannedDependencies.slice(1)) {
      ...
      context.add({
        ...
        relatedRefs: item.refs,
        suggestedResolution:
          "Link the event that causes or enables this scene, or mark it as a deliberate new sequence opening.",
      });
    }
  }
  ...
  for (const duplicates of beatSignatures.values()) {
    if (
      duplicates.length < 2 ||
      normalizeText(
        jsonString((duplicates[0].content as JsonObject).function) ?? "",
      ).length < 5
    ) {
      continue;
    }
    context.add({
      ...
      suggestedResolution:
        "Differentiate the beats or merge the duplicate structural work.",
    });
  }

  if (!isManuscriptComplete(context.input)) return;
  const dormantThreshold =
    context.input.projectRules?.plot?.dormantThreadSceneCount ?? 8;
  for (const thread of context.input.plotThreads.filter(
    (value) =>
      value.isCurrent &&
      value.invalidatedAt === null &&
    ...
    if (
      ...
      context.add({
        ...
        relatedRefs: [
          reference("plot-thread", thread.id, thread.key, thread.title),
        ],
        suggestedResolution:
          "Resume and resolve the thread, or mark it intentionally abandoned.",
      });
    }
    if (thread.kind === "main" && thread.status !== "resolved") {
      context.add({
        ...
        suggestedResolution:
          "Resolve the main dramatic question or revise the thread status with evidence.",
      });
    }
  }
}
...
function runPacingRules(context: DiagnosticContext): void {
  ...
  const chapterCounts = context.chapters.map((chapter) => ({
    ...
    count:
      ...
      chapter.scenes.reduce((sum, scene) => sum + wordCount(scene.content), 0),
  }));
    ...
    if (manuscriptMedian && manuscriptMedian > 0) {
      for (const item of usable) {
        ...
        context.add({
          code:
            ratio < lowRatio
              ? "abruptly-short-chapter"
              : "abruptly-long-chapter",
          category: "pacing",
          severity: "info",
          ...
          suggestedResolution:
            "Confirm the size change is intentional or rebalance the chapter boundary.",
        });
      }
    }
  }
  ...
  for (const scene of context.scenes) {
    ...
    context.add({
      ...
      relatedRefs: [sceneReference(scene)],
      suggestedResolution:
        "Confirm the scene earns its current length or revise the plan/boundary.",
    });
  }
  ...
  const flushLowConflict = () => {
    ...
    context.add({
      ...
      suggestedResolution:
        "Add meaningful pressure, compress the run, or confirm it is a deliberate recovery sequence.",
    });
    lowConflict.splice(0);
  };
  for (const scene of context.scenes) {
    const hasPlanning = Boolean(
      scene.sceneFunction.trim() || scene.goal.trim() || scene.outcome.trim(),
    );
    const hasPressure = Boolean(
      scene.obstacle.trim() || scene.conflict.trim() || scene.stakes.trim(),
    );
  ...
  const flushRevelations = () => {
    if (revelations.length >= revelationThreshold) {
      context.add({
        ...
        evidence: revelations.map((scene) =>
          sceneEvidence(scene, { quote: scene.revelation }),
        ),
        relatedRefs: revelations.map(sceneReference),
        suggestedResolution:
          "Check whether readers have enough consequence and processing time between revelations.",
      });
    }
  ...
  for (const scene of context.scenes) {
    if (enforceCompleteness) {
      const missing = [
        ...
        rules.requireSceneFunction && !scene.sceneFunction.trim()
          ? "scene function"
          : null,
      ...
      if (missing.length > 0) {
        context.add({
          ...
          relatedRefs: [sceneReference(scene)],
          suggestedResolution:
            "Fill the required fields or relax the project’s metadata policy.",
        });
      }
    }
    ...
    if (
      scene.content.trim() &&
      Math.abs(scene.actualWordCount - computedWords) > 1
    ) {
      context.add({
        ...
        relatedRefs: [sceneReference(scene)],
        suggestedResolution:
          "Recompute and persist the scene word count from the current prose branch.",
      });
    }
  }
}
...
function runPublishingRules(context: DiagnosticContext): void {
  ...
  const numbers = groupBy(context.chapters, (chapter) =>
  ...
  for (const chapters of numbers.values()) {
    ...
    context.add({
      ...
      suggestedResolution:
        "Assign each manuscript chapter a unique publication number.",
    });
  }

  if (
    rules.requireSequentialChapterNumbers !== false &&
    ...
    if (missing.length > 0) {
      context.add({
        ...
        suggestedResolution:
          "Renumber chapters sequentially or explicitly configure nonstandard numbering.",
      });
    }
  }

  if (rules.requireUniqueChapterTitles) {
    const titles = groupBy(context.chapters, (chapter) =>
    ...
    for (const chapters of titles.values()) {
      ...
      context.add({
        ...
        relatedRefs: chapters.map(chapterReference),
        suggestedResolution:
          "Give the chapters distinct titles or disable the uniqueness requirement.",
      });
    }
  }
  ...
  for (const chapter of context.chapters) {
    ...
    if (count === 0) {
      context.add({
        ...
        suggestedResolution:
          "Add prose or exclude the chapter from the publication build.",
      });
    }
    if (rules.requireFinalChapterStatus && chapter.status !== "final") {
      context.add({
        ...
        relatedRefs: [chapterReference(chapter)],
        suggestedResolution:
          "Complete the revision/proof pass and mark the chapter final before export.",
      });
    }
  }
    ...
    (sum, chapter) =>
      ...
      (wordCount(chapter.content) ||
        chapter.scenes.reduce(
          (sceneSum, scene) => sceneSum + wordCount(scene.content),
          0,
        )),
    0,
  );
  if (
    rules.targetWordCountMin !== undefined &&
    totalWords < rules.targetWordCountMin
  ) {
    context.add({
      ...
      relatedRefs: context.chapters.map(chapterReference),
      suggestedResolution:
        "Confirm the intended length or complete the missing manuscript work.",
    });
  }
  if (
    rules.targetWordCountMax !== undefined &&
    totalWords > rules.targetWordCountMax
  ) {
    context.add({
      ...
      relatedRefs: context.chapters.map(chapterReference),
      suggestedResolution:
        "Confirm the intended length or target structural cuts before export.",
    });
  }

  const activeTypes = new Set(
    context.activeArtifacts.map((artifact) => artifact.type),
  );
  for (const requiredType of rules.requiredArtifactTypes ?? []) {
    ...
    context.add({
      ...
      evidence: context.chapters
        .slice(0, 1)
        .map((chapter) => chapterEvidence(chapter)),
      relatedRefs: [],
...
function factInterval(
  ...
  return {
    start:
      fact.validFromOrder ??
      (fact.validFromSceneId
        ? sceneRank(context, fact.validFromSceneId)
        : Number.NEGATIVE_INFINITY),
    end:
      fact.validToOrder ??
      (fact.validToSceneId
        ? sceneRank(context, fact.validToSceneId)
        : Number.POSITIVE_INFINITY),
  };
}
...
function stateInterval(
  ...
  if (state.storyOrder !== null)
    return { start: state.storyOrder, end: state.storyOrder };
  return {
    start:
      state.validFromOrder ??
      (state.validFromSceneId
        ? sceneRank(context, state.validFromSceneId)
        : Number.NEGATIVE_INFINITY),
    end:
      state.validToOrder ??
      (state.validToSceneId
        ? sceneRank(context, state.validToSceneId)
        : Number.POSITIVE_INFINITY),
  };
}
...
function currentStateAt(
  ...
  return (
    states
      ...
      .sort(
        (left, right) =>
          stateCurrentStart(right, context) - stateCurrentStart(left, context),
      )[0] ?? null
  );
}

function stateCurrentStart(
  state: EntityState,
  context: DiagnosticContext,
): number {
  return (
...
function isDeadValue(value: JsonValue, stateKey: string): boolean {
  ...
  return (
    ["dead", "deceased", "killed"].includes(normalized) ||
    (value === true && /dead/.test(normalizeKey(stateKey)))
  );
}

function normalizeColor(value: string): string {
  return value.toLocaleLowerCase("en-US") === "grey"
    ? "gray"
...
function locationName(context: DiagnosticContext, locationId: string): string {
  return (
    context.locationById.get(locationId)?.name ?? `location “${locationId}”`
  );
}
...
function buildTravelMap(rules: TravelTimeRule[]): Map<string, number> {
  ...
  for (const rule of rules) {
    if (!Number.isFinite(rule.minimumMinutes) || rule.minimumMinutes < 0)
      continue;
...
function canonicalKnowledge(
  ...
  for (const fact of context.input.canonFacts.filter(
    ...
    const values = Array.isArray(fact.object)
      ...
      : [jsonString(fact.object)].filter((value): value is string =>
          Boolean(value),
        );
...
function sceneKnowledgeDeltas(
  ...
  const append = (
    value: JsonValue,
    defaultOperation: "gain" | "lose" = "gain",
  ) => {
    if (!isJsonObject(value)) return;
    const characterId =
      jsonString(value.characterId) ?? jsonString(value.character);
    const knowledgeKey =
      jsonString(value.knowledgeKey) ??
      jsonString(value.factKey) ??
      jsonString(value.knowledge);
    const operationText = normalizeKey(
      jsonString(value.operation) ?? jsonString(value.op) ?? defaultOperation,
    );
  ...
  if (Array.isArray(raw)) raw.forEach((value) => append(value));
    ...
    for (const key of ["gained", "learned", "added"]) {
      ...
      if (Array.isArray(values))
        values.forEach((value) => append(value, "gain"));
    }
    for (const key of ["lost", "forgotten", "removed"]) {
      const values = raw[key];
      if (Array.isArray(values))
        values.forEach((value) => append(value, "lose"));
    }
  }
  return uniqueBy(
    result,
    (value) => `${value.characterId}:${value.knowledgeKey}:${value.operation}`,
  );
}
...
function sceneObjectTransfers(
  ...
  for (const candidate of candidates) {
    ...
    const objectId =
      jsonString(candidate.objectId) ??
      jsonString(candidate.itemId) ??
      jsonString(candidate.entityId);
    const fromOwnerId =
    ...
    if (objectId && fromOwnerId && toOwnerId)
      result.push({ objectId, fromOwnerId, toOwnerId });
  }
  return uniqueBy(
    result,
    (transfer) =>
      `${transfer.objectId}:${transfer.fromOwnerId}:${transfer.toOwnerId}`,
  );
}

function sceneKnowledgeClaims(
  scene: DiagnosticSceneSnapshot,
): DiagnosticKnowledgeClaim[] {
  ...
  return uniqueBy(
    result,
    (value) =>
      `${value.characterId}:${value.knowledgeKey}:${value.start ?? ""}`,
  );
}
...
function linkedEvidence(
  ...
  if (sceneId && context.sceneById.has(sceneId))
    evidence.push(sceneEvidence(context.sceneById.get(sceneId)!));
...
function compareDependencyOrder(
  left: PlannedDependency,
  right: PlannedDependency,
): number {
...
function sceneDependencies(context: DiagnosticContext): PlannedDependency[] {
  const planById = new Map(
    context.activeArtifacts
      .filter((artifact) => artifact.type === "scene-plan")
      .map((artifact) => [artifact.id, artifact]),
  );
  const chapterNumbers = new Map(
    context.chapters.map((chapter) => [chapter.id, chapter.number]),
  );
  const plannedChapterNumbers = new Map(
    context.activeArtifacts.flatMap((artifact) => {
      if (artifact.type !== "chapter-brief" || !isJsonObject(artifact.content))
        return [];
      const content = artifact.content as JsonObject;
      return typeof content.number === "number" &&
        Number.isFinite(content.number)
        ? [
            [
              jsonString(content.chapterKey) ?? artifact.key,
              content.number,
            ] as const,
          ]
        : [];
  ...
  const live = context.scenes.map((scene) => {
    const plan = scene.sourceArtifactId
      ? planById.get(scene.sourceArtifactId)
      : undefined;
    const planContent =
      plan && isJsonObject(plan.content)
        ? (plan.content as JsonObject)
        : undefined;
    const key =
      jsonString(scene.metadata?.sceneKey) ??
      (planContent ? jsonString(planContent.sceneKey) : null) ??
      plan?.key ??
      scene.id;
    return {
      id: scene.id,
      ...
      return [
        {
          ...
          chapterOrder:
            plannedChapterNumbers.get(jsonString(content.chapterKey) ?? "") ??
            0,
          order:
            typeof content.ordinal === "number" &&
            Number.isFinite(content.ordinal)
              ? content.ordinal
  ...
  return [...live, ...plans].sort(
    (left, right) =>
      compareDependencyOrder(left, right) || left.id.localeCompare(right.id),
  );
}
