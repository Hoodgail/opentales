...
export function makeCleanDiagnosticsFixture(): StoryDiagnosticsInput {
  ...
  return snapshot({
    ...
    characters: [
      character("character-mara", "Mara"),
      character("character-elias", "Elias"),
    ],
    locations: [
...
export function makeTruePositiveDiagnosticsFixture(): StoryDiagnosticsInput {
  ...
  const scenes = [
    ...
    scene({
      ...
      dialogueTurns: [
        { speakerId: "character-elias", text: exposition, quote: exposition },
      ],
    }),
    scene({
      id: "scene-4",
    ...
    scene({
      ...
      goal: "",
      content:
        "A brief unplanned transition remains in the publication branch.",
    }),
    scene({
      id: "scene-6",
      chapterId: "chapter-3",
...
export function makeFalsePositiveDiagnosticsFixture(): StoryDiagnosticsInput {
  ...
  const input = snapshot({
    ...
    artifacts: [
      narrativeContractArtifact(),
      worldBibleArtifact(),
      characterBibleArtifact(["bridge-is-alive"]),
    ],
    canonFacts: [
      canonFact({
        id: "fact-gray-old",
...
function snapshot(
  overrides: Partial<StoryDiagnosticsInput>,
): StoryDiagnosticsInput {
  return {
...
function scene(
  overrides: Partial<DiagnosticSceneSnapshot> &
    Pick<DiagnosticSceneSnapshot, "id" | "chapterId" | "order" | "title">,
): DiagnosticSceneSnapshot {
  const {
    id,
    chapterId,
...
export function artifact(
  overrides: Pick<StoryArtifact, "id" | "key" | "title" | "type" | "content"> &
    Partial<StoryArtifact>,
): StoryArtifact {
  ...
  return artifact({
    ...
    content: {
      rules: [
        {
          key: "memory-price",
          statement: "Every map alteration consumes one living memory.",
        },
      ],
      institutions: [],
      geography: [],
