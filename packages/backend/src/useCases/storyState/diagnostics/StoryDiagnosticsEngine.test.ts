...
describe("StoryDiagnosticsEngine", () => {
  ...
  it("recognizes isolated character-bible references without publishing canonical characters", () => {
    ...
    input.characters = [];
    input.artifacts = ["mara", "elias"].map((name) =>
      artifact({
        id: `bible-${name}`,
        key: `character-${name}`,
        title: name,
        type: "character-bible",
        content: { characterKey: `character-${name}`, name },
      }),
    );
    input.chapters[0]!.scenes[0]!.povCharacterId = "bible-mara";
    const unknown = () =>
      runStoryDiagnostics(input).filter(
        (item) => item.code === "unknown-character-reference",
      );
  ...
  it("orders planned causal dependencies by chapter before chapter-local scene ordinal", () => {
    ...
    input.artifacts = [
      artifact({
        ...
        title: "First",
        type: "chapter-brief",
        content: { chapterKey: "c1", number: 1 },
      }),
      artifact({
        id: "c2",
        key: "c2",
        title: "Second",
        type: "chapter-brief",
        content: { chapterKey: "c2", number: 2 },
      }),
      artifact({
        id: "s2",
        key: "s2",
    ...
    const orderingErrors = () =>
      runStoryDiagnostics(input).filter(
        (item) => item.code === "causal-predecessor-after-scene",
      );
    expect(orderingErrors()).toEqual([]);
    input.artifacts[2]!.content = {
      sceneKey: "s2",
      chapterKey: "c1",
      ordinal: 1,
      dependencies: ["s1"],
    };
    input.artifacts[3]!.content = {
      sceneKey: "s1",
      chapterKey: "c1",
      ordinal: 2,
      dependencies: [],
  ...
  it("detects every semantic and craft family required by the research document", () => {
    const diagnostics = runStoryDiagnostics(
      makeTruePositiveDiagnosticsFixture(),
    );
    ...
    for (const category of [
      ...
      expect(
        ...
        `missing diagnostic category ${category}`,
      ).toBe(true);
    }
  ...
  it("attaches stable IDs and navigable evidence or related references to every finding", () => {
    ...
    expect(new Set(first.map((diagnostic) => diagnostic.id)).size).toBe(
      first.length,
    );
    for (const diagnostic of first) {
      expect(diagnostic.id).toMatch(/^[a-f0-9]{24}$/);
      expect(
        diagnostic.evidence.length + diagnostic.relatedRefs.length,
      ).toBeGreaterThan(0);
  ...
  it("persists point-in-story entity state and flags a post-death appearance without flagging the death scene itself", () => {
    ...
    expect(
      findings[0].evidence.some((span) => span.sceneId === "scene-2"),
    ).toBe(true);
  });
  ...
  it("requires an explicit object transfer when sequential ownership states change", () => {
    ...
    input.entityStates = [
      ...
      entityState({
        ...
        sourceSpan: {
          chapterId: "chapter-2",
          sceneId: "scene-2",
          quote: "Mara held the key.",
        },
      }),
    ];

    expect(
      runStoryDiagnostics(input).map((diagnostic) => diagnostic.code),
    ).toContain("object-owner-changed-without-transfer");
    ...
    expect(
      runStoryDiagnostics(input).map((diagnostic) => diagnostic.code),
    ).not.toContain("object-owner-changed-without-transfer");
  });
  ...
  it("does not interpret character inventory descriptions as object owner identities", () => {
    ...
    input.entityStates = [
      entityState({
        ...
        stateKey: "possession",
        value: "Carries the spool inside her coat.",
        validFromOrder: 0,
        validToOrder: 0,
      }),
      entityState({
        ...
        entityId: "character-mara",
        stateKey: "possession",
        value: "Carries the spool and shared logbook.",
        validFromOrder: 1,
        validToOrder: null,
      }),
    ];
    expect(runStoryDiagnostics(input).map((item) => item.code)).not.toContain(
      "object-owner-changed-without-transfer",
    );
    input.entityStates[0]!.validToOrder = null;
    expect(runStoryDiagnostics(input).map((item) => item.code)).toContain(
  ...
  it("honors the pantser opt-out even when project metadata requirements are configured", () => {
    ...
    const codes = runStoryDiagnostics(input).map(
    ...
    expect(
      runStoryDiagnostics(input).map((diagnostic) => diagnostic.code),
    ).toContain("required-scene-metadata-missing");
  });
  ...
  it("supports category allowlists and per-code opt-outs without weakening unrelated diagnostics", () => {
    ...
    expect(
      diagnostics.every((value) =>
        ["continuity", "chronology"].includes(value.category),
      ),
    ).toBe(true);
    expect(diagnostics.map((value) => value.code)).not.toContain(
      "canon-conflict",
    );
    expect(diagnostics.map((value) => value.code)).toContain(
      "impossible-travel",
    );
  });
  ...
  it("keeps the exported category catalog unique and complete", () => {
    expect(new Set(STORY_DIAGNOSTIC_CATEGORIES).size).toBe(
      STORY_DIAGNOSTIC_CATEGORIES.length,
    );
  ...
  it("rejects a malformed runtime snapshot instead of emitting misleading partial diagnostics", () => {
    const input = makeCleanDiagnosticsFixture();
    expect(() =>
      runStoryDiagnostics({ ...input, chapters: null } as never),
    ).toThrow("Story diagnostics chapters must be an array");
  });
});
